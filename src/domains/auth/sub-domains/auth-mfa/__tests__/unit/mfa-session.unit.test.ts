...
} from '@/domains/auth/sub-domains/auth-mfa/mfa-session.js';
...
describe('mfa-session', () => {
  ...
  it('verifyMfaSession rejects payload with empty user_public_id', async () => {
    vi.mocked(redis.get).mockResolvedValue(JSON.stringify({ user_public_id: '' }));
    await expect(verifyMfaSession(redis, 'token-abc')).rejects.toBeInstanceOf(UnauthorizedError);
  });

  it('verifyMfaSession deletes the token on successful consume', async () => {
    vi.mocked(redis.get).mockResolvedValue(JSON.stringify({ user_public_id: 'user_public_abc' }));
    const payload = await verifyMfaSession(redis, 'token-success');
    expect(payload.user_public_id).toBe('user_public_abc');
    expect(redis.del).toHaveBeenCalledWith(`${MFA_SESSION_KEY_PREFIX}token-success`);
  });
  ...
  it('verifyMfaSession rejects replayed token after first successful consume', async () => {
    ...
    // Second call: state was deleted, Redis returns null
