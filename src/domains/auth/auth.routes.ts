...
export const authRoutesPlugin: FastifyPluginAsync = async (app) => {
  ...
  zodApplication.post('/webauthn/authenticate/options', {
    ...
    schema: {
      ...
      description:
        'Returns WebAuthn authentication options (challenge + allowed credentials) for a passwordless login ceremony. Public: the client echoes the response back to /webauthn/authenticate/verify to complete sign-in.',
      tags: ['Auth', 'WebAuthn'],
  ...
  zodApplication.post('/webauthn/authenticate/verify', {
    ...
    schema: {
      ...
      description:
        'Verifies the WebAuthn assertion from an authentication ceremony and, on success, returns access and refresh tokens and sets the session cookie. Public login-flow endpoint.',
      tags: ['Auth', 'WebAuthn'],
