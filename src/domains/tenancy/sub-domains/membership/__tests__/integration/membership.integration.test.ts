...
describe('Membership Sub-Domain — Integration', () => {
  ...
  // every people-sharing entry point with a 422 UnprocessableEntityError (the org type is
  // immutable, so retrying the identical request always fails — 422, not 409). Unit-tested in
  ...
  describe('PERSONAL organization single-member guard (HTTP-level coverage)', () => {
    ...
    // either form so the assertion is robust to test-ordering — the 422 + `code:
    // 'unprocessable_entity'` are the load-bearing contract, and either detail value uniquely
    // identifies this guard.
    function expectPersonalNoMembersUnprocessable(response: {
      statusCode: number;
      json: () => unknown;
    }) {
    ...
    it('rejects POST /memberships on a PERSONAL org with 422 (errors:personalOrganizationNoMembers) and creates no row', async () => {
      ...
      expectPersonalNoMembersUnprocessable(response);
