...
describe('MemberRoleRepository (database)', () => {
  ...
  it('paginates an empty organization role list', async () => {
    ...
    const emptyPage = await repository.findByOrganizationId(organization.id, { limit: 20, order: 'asc' });
  ...
  it('finds roles by organization and public id', async () => {
    ...
    const page = await repository.findByOrganizationId(organization.id, { limit: 20, order: 'asc' });
