...
describe('organization-api-key-tombstone-retention.processor boundary cases', () => {
  beforeEach(() => {
  ...
  it('purges only tombstones older than the cutoff', async () => {
    ...
    const result = await runOrganizationApiKeyTombstoneRetentionJob({} as never);
  ...
  it('logs purge counts on completion', async () => {
    deleteInBatchesByConditionMock.mockResolvedValue({ deletedCount: 6, blockedCount: 0 });

    await runOrganizationApiKeyTombstoneRetentionJob({} as never);

    expect(loggerInfoMock).toHaveBeenCalledWith(
      expect.objectContaining({ deletedCount: 6, blockedCount: 0, retentionDays: 30 }),
