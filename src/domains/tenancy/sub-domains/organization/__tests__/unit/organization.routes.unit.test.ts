...
describe('organizationRoutes — route configuration (sec-new-M1)', () => {
  ...
  it('POST /organizations/:organization_id/api-keys/:api_key_id/rotate retains its rateLimit config', async () => {
    const { app: testApp, capturedRoutes } = await buildTestApp();
    app = testApp;

    const route = capturedRoutes.get(
      'POST /organizations/:organization_id/api-keys/:api_key_id/rotate',
    );
    const config = route?.config as Record<string, unknown> | undefined;
