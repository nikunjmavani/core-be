...
describe('Integration: OpenAPI response validation', () => {
  ...
  it('GET /readyz response includes OpenAPI-required fields', async () => {
    const response = await injectUnauthenticated(app, { method: 'GET', url: '/readyz' });
    expect(response.statusCode).toBe(200);

    const required = requiredFieldsForPath(spec, '/readyz', 'get', '200');
