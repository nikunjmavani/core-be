...
describe('Security: Tenant RLS concurrency', () => {
  ...
  it('should return correct organization data under parallel mixed tenant requests', async () => {
    ...
    for (let index = 0; index < 20; index += 1) {
      ...
      responses.push(
        await request
          .get(`/api/v1/tenancy/organizations/${tenant.organization.public_id}`)
          .set('Authorization', `Bearer ${tenant.token}`)
          .set(organizationHeaders(tenant.organization.public_id)),
      );
    }
    ...
    for (let index = 0; index < responses.length; index++) {
      ...
      const body = response.body as { data?: { id?: string; public_id?: string } };
