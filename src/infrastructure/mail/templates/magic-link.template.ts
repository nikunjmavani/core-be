 ...
 *
 * @remarks
 * `magicLinkUrl` is server-generated but is passed through {@link escapeHtml} so
 * `&` query-string delimiters are correctly encoded as `&amp;` in the HTML attribute
 * and text node, and to guard against injection if URL composition ever changes.
 */
export function magicLinkTemplate(data: MagicLinkTemplateData): string {
  ...
  return baseTemplate({
    ...
    body: `
      ...
      <p style="font-size: 12px; color: #9ca3af; word-break: break-all;">${magicLinkUrl}</p>
    `,
  });
}
