...
- **Network I/O outside RLS contexts**: callers must not invoke Stripe inside `withOrganizationDatabaseContext`. Enforced by an ESLint guardrail and called out as an invariant in [src/domains/billing/sub-domains/subscription/OVERVIEW.md](src/domains/billing/sub-domains/subscription/OVERVIEW.md).
