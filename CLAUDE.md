...
- **Config**: Environment variables from `src/shared/config/env.config.ts`. Env files are **root only**: `.env` then optional `.env.local` (overrides; both not committed), `.env.example` and `.env.local.example` (committed templates); no `env/` directory. Load order: `src/shared/config/load-env-files.ts`.
