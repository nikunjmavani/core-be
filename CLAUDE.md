...
- `pnpm db:viewer` — local DB Schema Viewer (`tooling/db-viewer/`): loopback-only ER board for Drizzle schemas + `migrations/*.sql` replay; per-boot session token in the printed URL
