...
- Headers: `Authorization: Bearer`, `X-Organization-Id`, `X-Idempotency-Key` (required on the 10 `idempotencyRequired` writes), `X-Captcha-Token` (public auth forms), `X-CSRF-Token` (refresh only), `Stripe-Signature` (Stripe-sent); ecosystem X- forms kept (`X-Request-Id`, `X-Api-Key`, `X-RateLimit-*`, …)
...
- `pnpm validate:route-params-schema` — verify every route whose path carries a `:param` segment declares a `schema.params` Zod schema (boundary rejection + OpenAPI param contract; controllers still run `validatePublicIdParam`)
