...
| [`session-start.sh`](session-start.sh) | Claude `SessionStart` | On the web, verifies Node/deps/codegraph, installs deps (switching to a new-enough Node via `$CLAUDE_ENV_FILE` when needed), and runs `pnpm agent-os:check` as the **only** startup gate (fail-open, web-only); injects the skill-trigger routing map + an env/commands summary as `additionalContext`. Heavy work (`compose:up`, migrate, seed, tests, `pnpm dev`) runs on demand per prompt, not at startup. Runs synchronously. |
