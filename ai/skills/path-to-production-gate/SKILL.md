...
### 2. Run additional codebase checks
...
If new production-readiness checks are added to the codebase or to **production-hardening-guard**, ensure this skill's "Additional codebase checks" (step 2) and the plan template stay in sync so the full code is reviewed before each path to production.
