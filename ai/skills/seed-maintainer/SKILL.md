...
- **`src/scripts/seed/bulk.ts`** — `MODULES` (one `DomainSeedModule` per domain), `orderModules` (topological by `dependsOn`), `runBulkSeed` (guard → resolve → all `seedReference`, then all `seedBulk`), `closeDatabase()` in `finally`.
...
3. **Compose up.** Ensure the parent `seed/index.ts` includes the new contribution via `composeContributions(...)`. If a new top-level domain owns tables, export a `DomainSeedModule` (`name` + `dependsOn`) and register it in `MODULES` in `bulk.ts`.
