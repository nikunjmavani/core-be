...
| 4d   | `pnpm features:generate` + `git add src/**/DOCS.md` + `pnpm features:check:strict` | Layered feature-doc gate. Regenerates per-folder `DOCS.md` and `src/DOCS.md`, then runs the strict baseline ratchet. Fails on drift OR if any of the four `MISSING_*` token counts exceeds the locked baseline at `tooling/feature-docs/missing-tokens.baseline.json`. Resolve via **feature-doc-maintainer** (which dispatches to **tsdoc-export-guard** / **route-schema-doc-guard** / **overview-doc-maintainer** / **system-narrative-maintainer**). |
