...
2. **Use `pnpm.overrides` for transitive vulnerabilities**
   ...
   - Run `pnpm install --no-frozen-lockfile` to refresh the lockfile, then `pnpm audit` again. Commit both `package.json` and `pnpm-lock.yaml`.
...
5. [ ] Commit `package.json` and `pnpm-lock.yaml` together; do not commit lockfile with failing audit.

## Reference

- **Overrides**: [pnpm overrides](https://pnpm.io/package_json#pnpmoverrides) in `package.json`.
