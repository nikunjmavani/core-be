...
- [ ] BullMQ stalled job configuration: `lockDuration`, `stalledInterval`, `maxStalledCount` in `src/infrastructure/queue/worker-options.ts`; all workers use shared options and log on `stalled` event
