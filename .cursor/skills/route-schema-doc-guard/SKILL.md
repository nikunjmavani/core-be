...
The two non-routes files that register HTTP endpoints are explicitly listed in [`tooling/openapi/extractors/route-schema-metadata.ts`](../../../tooling/openapi/extractors/route-schema-metadata.ts) (search for `EXTRA_ROUTE_SOURCE_FILES`). When adding a route in either of these files:
