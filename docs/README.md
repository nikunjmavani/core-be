...
**In-source layered documentation (lives under `src/`, not `docs/`):**

- System narratives — `src/OVERVIEW.md`, `src/PATTERNS.md`, `src/FLOWS.md`, `src/POLICIES.md` (hand-authored).
- Per-folder overviews — `src/<folder>/OVERVIEW.md` (hand-authored, four templates A.1–A.4).
- Per-folder index — `src/<folder>/DOCS.md` and `src/DOCS.md` (auto-generated by `pnpm features:generate`; ratchet-gated by `pnpm features:check:strict`).
- TSDoc on every public export, plus `@remarks` on services/workers/processors/policy files.

These are owned by the **feature-doc-maintainer**, **system-narrative-maintainer**, **overview-doc-maintainer**, **tsdoc-export-guard**, and **route-schema-doc-guard** skills (auto-trigger rule: `.cursor/rules/feature-doc-maintainer-sync.mdc`). Hand-written docs under `docs/` are still owned by **docs-maintainer**.
