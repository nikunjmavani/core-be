...
| [reference/architecture/realtime-in-core-be-plan.md](reference/architecture/realtime-in-core-be-plan.md) | **Proposal · CHOSEN** — live push with WebSockets inside the `core-be` Fastify process (Option A): cookie-authenticated upgrade, in-process registry, Redis Streams fan-out, multi-org tiering, and the two costs it accepts. |
| [reference/architecture/realtime-go-service-plan.md](reference/architecture/realtime-go-service-plan.md) | **Proposal · alternative** — the same feature via a standalone Go service (`core-rt`). Kept as the reference for outgrowing Option A; its Appendix C is the trade-off analysis that chose between them. |
| [reference/architecture/realtime-implementation-map.md](reference/architecture/realtime-implementation-map.md) | **Proposal** — the core-be file map for the above: every file to add/update, the gates that fire, and the build order. |
| [reference/architecture/realtime-go-setup-guide.md](reference/architecture/realtime-go-setup-guide.md) | **Proposal** — the Go side from scratch: toolchain install, dependencies, Makefile, Docker, CI, checkpoint-by-checkpoint walk, and a Go-for-TypeScript concept map. |
