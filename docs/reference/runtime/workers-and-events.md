...
**Typical flow:** `service` → `eventBus.emit` → handler → `recordOutboxEmail()` + post-commit `dispatchOutboxEmail()` → mail worker.

Worker/runtime paths without a request transaction call `recordOutboxEmail()` then `dispatchOutboxEmail()` directly. Both are valid.
...
1. **`buildApp()` → `registerEventHandlers()`** ([`register-event-handlers.ts`](../../../src/core/events/register-event-handlers.ts)) **before** routes. Handlers that only need the mail enqueue primitives (`recordOutboxEmail`/`dispatchOutboxEmail`) or have no container deps (auth + tenancy email handlers).
...
Event handlers read `event.requestId` and pass it to `dispatchOutboxEmail()`, `enqueueNotification()`, `enqueueWebhookDeliveryByAttemptId()`, etc. Job payloads store `requestId` (Zod-validated); workers log it on `mail.worker.processing` and **`email.sent`** so worker logs correlate with API `request.complete` (`requestId` field).
...
tenancy/sub-domains/member-invitation/events/*.ts  →  recordOutboxEmail() + dispatchOutboxEmail()
...
auth/sub-domains/auth-method/events/*.ts  →  recordOutboxEmail() + dispatchOutboxEmail()
