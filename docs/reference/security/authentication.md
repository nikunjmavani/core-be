...
**Invitation onboarding.** A user added to an org by email gets a pre-created **bare** account (no credential, `is_email_verified=false`) so the `INVITED` membership has a `user_id`. They onboard via any path above — signup **claims** the bare row (sets a password, email stays unverified), while magic-link / OAuth claim it **and flip `is_email_verified=true`** because completing either flow proves control of the inbox. Accepting the invitation (`POST /api/v1/tenancy/invitations/{invitation_id}/accept`) requires authentication, an email **matching** the invitee, **and a verified email** — so a forwarded invite token cannot be used to join by someone who merely password-claimed the address without proving email control. A claimer arriving via magic-link or OAuth clears the verified-email gate in the same step that claims the account.
