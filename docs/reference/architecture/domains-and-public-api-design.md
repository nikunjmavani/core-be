...
- [x] **db-drizzle-schema** — Add Drizzle schema definitions (snake*case) for auth.*, tenancy._, billing._, notify.\_, audit.\* in `src/db/schema.ts` or split per schema.
...
- [x] Convention: only root `.env.example` is committed; `.env.<environment>` (e.g. `.env.development`, `.env.production`) are gitignored operator templates generated by `pnpm env:init`; no `env/` directory
- [x] `.gitignore`: ignores all `.env*` except `.env.example`
- [x] `.env.example`: two top-level halves (`# GitHub Secrets`, `# GitHub Variables`) — section IS Secret/Variable classification for `pnpm env:sync`
- [x] PR check (`.github/workflows/pr-checks.yml`): reject commits that add any `.env.*` file other than `.env.example`
- [x] README / docs: local setup uses `pnpm env:init` (see [environment-variables.md](../../deployment/runbooks/environment-variables.md))
