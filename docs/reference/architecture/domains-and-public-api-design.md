...
- [x] Convention: only root `.env.example` is committed; `.env.<environment>` (e.g. `.env.development`, `.env.production`) are gitignored operator templates generated by `pnpm github:sync` from `.github/sync.config.json`; no `env/` directory
