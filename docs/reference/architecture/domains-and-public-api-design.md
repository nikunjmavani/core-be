...
An organization has an immutable `type` — `PERSONAL` (single-owner workspace) or `TEAM` (shareable, multi-member). There is **one** route surface for both: no personal-only or team-only URLs. Six capabilities are structurally unavailable to a personal organization, and every serialized organization response advertises them so clients can hide or disable the corresponding actions instead of probing.

- **`capabilities` object** — every organization response (get, list, create, patch) carries a `capabilities` object with booleans `can_invite_members`, `can_manage_members`, `can_manage_roles`, `can_transfer_ownership`, `can_delete`, `can_manage_billing`. These describe the **org type's** capability (`TEAM` → all `true`, `PERSONAL` → all `false`), **not** the caller's permission (permissions/roles govern that separately). Derived by `organizationCapabilities(type)` in `src/domains/tenancy/sub-domains/organization/organization-capability.ts`.
