...
- **Manual merge:** every Dependabot PR needs human review and merge per branch protection. Routine Dependabot version-update PRs target `dev`; reserve `main` dependency PRs for deliberate production or security fixes and back-merge them promptly. When **PR CI fails** on a Dependabot PR, [`.github/workflows/dependabot-ci-triage.yml`](../../.github/workflows/dependabot-ci-triage.yml) opens or updates a triage issue.
