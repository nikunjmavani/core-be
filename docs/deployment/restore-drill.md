...
Provision in `.env.development` / `.env.production` (GitHub Secrets half) and sync with `pnpm github:sync`. The workflow resolves Neon project **`core-be`** by name via the Neon API and uses the GitHub Environment resolved from the git ref (single trunk: `main` → `production`; environment ≠ branch, there is no `dev` branch).
