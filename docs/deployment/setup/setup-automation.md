...
| Command                             | Description                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm setup --init`                 | **Optional first step** — interactive: ask organization, project name, environments → generate `setup.config.json` and a `.setup/.setup-credentials` template. No JSON editing.                                                                                                                                                                                                                   |
| `pnpm setup:infra`                  | **Run first** (after init and filling secrets) — full provisioning. Fill `.setup/.setup-credentials` (each line has a comment with the URL to get the key). Creates all resources (Neon, Redis, S3, Sentry, Railway, GitHub). Double confirmation, pre-existence check, atomic rollback on failure. Required before auto-deploy works.                                                            |
| `pnpm setup:infra:check`            | Health check. Verifies all provisioned resources are reachable. Run after setup or when debugging.                                                                                                                                                                                                                                                                                 |
| `pnpm setup:infra:status`           | Status report from the in-memory run state (empty unless hydrated from remote). For authoritative provisioned-vs-missing truth, use `pnpm setup:infra:inspect`.                                                                                                                                                                                                                                                                   |
| `pnpm setup:infra:update`           | Re-sync GitHub branches, rulesets, environments, and secrets from `.setup/.setup-credentials`. Use after rotating a key.                                                                                                                                                                                                                                                                          |
...
| `pnpm setup:infra:delete`           | Print a manual-delete guide: dashboard URLs and resource identifiers for every provider with provisioned state (run after a provision/reconstruct so the in-memory state holds the ids). **The script never deletes resources** — open each dashboard and remove items by hand. `setup:infra:revert` is kept as a back-compat alias for this command. |
...
2. **Secrets:** Fill `.setup/.setup-credentials` (each variable has a comment with the URL to get the key). Run `pnpm setup:infra`; browser can open for each provider — paste values into `.setup/.setup-credentials`.
...
- **Regenerate anytime:** Run `pnpm setup:infra:export-env` to refresh all `.env.<environment>` files from `setup.config.json` + `.setup/.setup-credentials`. Provisioned values already in each env file are **preserved** (state is ephemeral, so export-env never blanks a value it can't rebuild); a full `pnpm setup:infra` run refreshes them from the live provisioning flow.
...
- **Secrets:** `.setup/.setup-credentials` at repo root (`KEY=value`, comments with URLs). `process.env` is merged so `NEON_API_KEY=... pnpm setup:infra` works.
- **Tools:** One API key per provider (or CLI auth where supported); the script maps env vars to Neon, Redis, AWS, Railway, etc.

| Command                    | Purpose                                             |
| -------------------------- | --------------------------------------------------- |
| `pnpm setup --init`        | Interactive config + optional `.setup/.setup-credentials` template |
...
- [setup-token-instructions.md](setup-token-instructions.md) — Where to get each token; env var names for `.setup/.setup-credentials`
