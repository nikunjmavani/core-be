...
| `pnpm setup:infra:delete`           | Print a manual-delete guide: dashboard URLs and resource identifiers for every provider with provisioned state (run after a provision/reconstruct so the in-memory state holds the ids). **The script never deletes resources** — open each dashboard and remove items by hand. |
