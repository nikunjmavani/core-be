...
| `JWT_SECRET`             | Min 32 chars (HS256 in test)    | **RS256:** `JWT_PUBLIC_KEY` + `JWT_PRIVATE_KEY` (PEM) — required in production |
...
| `DEPLOYMENT_PROCESS_COUNT` | Unset (local defaults to 1 API + 1 worker) | **Required.** `api_replicas + worker_replicas` on **both** API and worker services |
| `DEPLOYMENT_API_PROCESS_COUNT` / `DEPLOYMENT_WORKER_PROCESS_COUNT` | Optional split counts | Alternative to `DEPLOYMENT_PROCESS_COUNT` when explicit API/worker counts are clearer |
| `DB_MAX` / `POSTGRES_RESERVED_CONNECTIONS` | Defaults (`10`) | Tune pool size and headroom — [resource-limits.md](resource-limits.md) |
...
| `JWT_PRIVATE_KEY` / `JWT_PUBLIC_KEY` | RS256 key pair (required in production per env schema)                                          |
...
| `AUDIT_RETENTION_DAYS`               | Required in production                                                                          |
| `SESSION_RETENTION_DAYS`             | Required in production                                                                          |
| `DEPLOYMENT_PROCESS_COUNT`           | **Required** — `api_replicas + worker_replicas` (same value on API and worker services)         |
| `DEPLOYMENT_API_PROCESS_COUNT` / `DEPLOYMENT_WORKER_PROCESS_COUNT` | Alternative split counts when not using `DEPLOYMENT_PROCESS_COUNT` |
| `DB_MAX`                             | Optional; default `10` — size with [resource-limits.md](resource-limits.md) connection budget   |
...
5. **Health:** `GET /health/live` (liveness), `GET /health/ready` (readiness)
...
| ------ | ---------------------------------------------------------------------------------------- | ------------------ |
| `dev`  | [deploy-railway.yml](../../../.github/workflows/deploy-railway.yml) (after CI on `dev`)  | `dev`              |
| `qa`   | [deploy-railway.yml](../../../.github/workflows/deploy-railway.yml) (after CI on `qa`)   | `qa`               |
| `main` | [deploy-railway.yml](../../../.github/workflows/deploy-railway.yml) (after CI on `main`) | `production`       |

**Not automated by deploy workflows:** DB migrations, worker service, optional integration env (`RESEND_*`, `STRIPE_*`, etc.) — configure on Railway or extend workflows.

Deploy **two** services: API (`Dockerfile` / `api` target) and worker (`Dockerfile.worker`). Health probe: `GET /health/ready` on API.
...
```bash
curl -sf https://YOUR_API/health/live
curl -sf https://YOUR_API/health/ready
```
...
| Hosted deploy  | Merge to `dev` / `qa` / `main` → deploy workflow                            |
...
Before go-live, confirm the managed Redis instance (Upstash, Railway, etc.):
...
2. **Deploy** API + worker; confirm `GET /health/ready` returns `ok` for database, redis, and bullmq.
3. **Smoke** — `pnpm test:api-smoke` against staging base URL (or minimal billing/auth checks).
4. **Drain** — send `SIGTERM` to API; confirm `/health/ready` returns `503` with `status: "draining"` while `/health/live` stays `200` until exit (see [resource-limits.md](resource-limits.md)).
