...
| Per-user PENDING quota | `countPendingByUserId(user.id)` must be `< UPLOAD_MAX_PENDING_PER_USER` (default 100). Stops a single authed user from looping create-without-confirm and exhausting storage. Reconciled by the sweeper (see below).                  |
