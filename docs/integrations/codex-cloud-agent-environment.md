...
- **Pre-pull in setup, start in task** — [`codex-setup.sh`](../../tooling/setup/agent/codex-setup.sh) runs [`install-docker.sh`](../../tooling/setup/agent/install-docker.sh) and [`install-docker-images.sh`](../../tooling/setup/agent/install-docker-images.sh) by default, so Codex uses a preinstalled Docker when available, installs Docker CLI/Compose when missing, and pulls images while network is up through the same mirror path as Claude Cloud. During the task, [`bootstrap.sh`](../../tooling/setup/agent/bootstrap.sh) runs [`ensure-docker-daemon.sh`](../../tooling/setup/agent/ensure-docker-daemon.sh) immediately before compose so a stopped daemon is started before `SONAR=0 pnpm compose:up && pnpm compose:wait && pnpm db:migrate`.
