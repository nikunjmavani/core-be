...
- **Branch creates are allowed; branch deletes are not.** On Claude Code web the git proxy does **not** pin a session to its assigned `claude/<slug>` branch — a push under any conventional `<type>/<slug>` name succeeds — but every form of branch delete is refused, so a `claude/*` branch that has already been pushed cannot be removed from inside the session. Rename with `git branch -m <type>/<slug>` **before** the first push, per [git-branch-naming.mdc](../../agent-os/rules/git-branch-naming.mdc), which keeps `claude/*` allowlisted as the fallback. Verify the equivalent behaviour per platform rather than assuming parity — the Claude Code web specifics live in [claude-code-web-environment.md](claude-code-web-environment.md).
