...
Run `pnpm env:init` once to generate `.env.development` and `.env.production`
(both **gitignored**) from the committed `.env.example` template. Then edit
each file with the values from the per-provider sections below. Push to GitHub
with `pnpm env:sync <environment>` when ready. **Never commit any `.env.*`
...
flowchart TB
  ...
  required --> env_file[".env.&lt;environment&gt; at project root\n(gitignored, generated by pnpm env:init)"]
  optional --> env_file
  env_file -- pnpm env:sync --> github[GitHub Environment\n(Secrets + Variables)]
```

Required vs optional credentials and where they go: local
`.env.<environment>`, then pushed to GitHub Environments by `pnpm env:sync`,
...
`pnpm env:init`). Keep each key under the half + sub-section it already lives
in — that placement is what `pnpm env:sync` reads when classifying. The
...
```bash
pnpm env:sync development --dry-run    # preview
pnpm env:sync development              # push (creates the env, gh secret set / gh api variables)
```
