...
Neon **project ID** is resolved automatically via the Neon API using project name **`core-be`** (same lookup as `core-infra`'s `pnpm setup:infra`). The workflow maps **`main` → `production`**, **`dev` → `development`**. Without the API key in the matching GitHub Environment, the monthly workflow **fails**.
