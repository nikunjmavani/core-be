...
# `dist/` already contains every runtime asset: the build stage runs `pnpm build`, which ends in
# tooling/build/copy-dist-assets.mjs. Re-copying them here would be a second, silently divergent
# source of truth — the script is the only one.
