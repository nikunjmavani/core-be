...
import { hasGithubToken } from './env-secrets.js';
...
function buildGuideSteps(config: SetupConfig): GuideStepDefinition[] {
  ...
  const steps: GuideStepDefinition[] = [
    ...
    },
    {
      ...
      secretsCheck: (secrets) => isSecretFilled(secrets.upstash.redisUrl),
      browserUrls: ['https://console.upstash.com/'],
      instructions: [
        '1. Log in to Upstash (or sign up at upstash.com)',
        '2. Create one Redis database for cache, rate limits, idempotency, permission cache, and BullMQ',
        '3. Copy its Redis URL (rediss://... or redis://...) and set UPSTASH_REDIS_URL in ' +
          secretsPath,
        '',
        '4. Save the file',
      ],
