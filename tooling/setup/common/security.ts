 ...
 * - File permission check: warns if .setup/.setup-credentials or .env.<env> are world-readable
 */
...
export function runSecurityChecks(envSetupPath: string, envFiles: string[]): boolean {
  ...
  // Check .setup/.setup-credentials permissions
  const setupPerms = checkFilePermissions(envSetupPath);
  if (!setupPerms.safe) {
    logger.warn(
      `.setup/.setup-credentials has permissions ${setupPerms.mode} (should be 600). Run: chmod 600 ${envSetupPath}`,
    );
    allSafe = false;
  }
