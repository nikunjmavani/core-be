...
const SIMPLE_VARS: Array<[string, string]> = [
  ...
  ['UPSTASH_REDIS_URL', TOKEN_URLS.UPSTASH_REDIS_URL],
...
/**
 * Build SetupSecrets from env-style source (.env.setup + process.env).
 * Only returns a full object; missing required keys are empty strings (caller can check isSecretFilled).
 */
export function loadSecretsFromEnv(environmentNames: string[]): SetupSecrets {
  ...
  return {
    ...
    upstash: {
      redisUrl: get(source, 'UPSTASH_REDIS_URL'),
    },
    aws: {
...
/** Read a single variable value from .env.setup (and process.env). Returns empty string if missing. */
...
/**
 * Set a variable in .env.setup: replace the line starting with KEY= or append if missing.
 * Use after init when user provides a value (e.g. NEON_ORG_ID).
 */
...
/** Whether any secret is provided via env / .env.setup. */
export function hasAnyEnvSecret(environmentNames: string[]): boolean {
  ...
  return (
    ...
    filled(secrets.upstash.redisUrl) ||
...
/**
 * Load .env.setup into process.env so child processes (e.g. gh CLI) see GITHUB_TOKEN etc.
 * Call once at startup before any provision/update steps.
 */
...
/**
 * Build .env.setup template content with project/org/env header and per-variable comments (URL where to get each key).
 */
export function buildEnvSetupTemplateContent(config: SetupConfig): string {
  ...
  const lines: string[] = [
    '# Setup secrets — fill each value and run pnpm setup:infra.',
    '# Get each key from the URL on the line above the variable.',
    '',
    '# Project / organization / environments (to change these, edit tooling/setup.config.json and re-run pnpm setup:infra:init)',
  ...
  lines.push('# Optional OAuth per env (e.g. dev):');
...
/** Write .env.setup template only when file does not exist. Never overwrites existing variables. */
...
/**
 * Append any missing template variables (e.g. GITHUB_TOKEN) to existing .env.setup.
 * Only appends; never overwrites existing keys or values.
 * Call after init when .env.setup already exists so new template keys get added.
 */
...
const UPSTASH_BLOCK = `# ${TOKEN_URLS.UPSTASH_REDIS_URL}
UPSTASH_REDIS_URL=
`;

/** Replace old Redis Cloud vars (REDIS_ACCOUNT_KEY, REDIS_SECRET_KEY) with UPSTASH_REDIS_URL in content. */
function migrateRedisToUpstashInContent(content: string): string {
  if (!content.includes('REDIS_ACCOUNT_KEY') && !content.includes('REDIS_SECRET_KEY'))
    return content;
  ...
  while (i < lines.length) {
    const line = lines[i];
    if (line.trim() === 'REDIS_ACCOUNT_KEY=' || line.trim().startsWith('REDIS_ACCOUNT_KEY=')) {
      const next = lines[i + 1];
      if (
        next !== undefined &&
        (next.trim() === 'REDIS_SECRET_KEY=' || next.trim().startsWith('REDIS_SECRET_KEY='))
      ) {
        out.push(UPSTASH_BLOCK.trim());
        i += 2;
        continue;
      }
    }
    if (line.trim() === 'REDIS_SECRET_KEY=' || line.trim().startsWith('REDIS_SECRET_KEY=')) {
      i += 1;
      continue;
    }
    out.push(line);
...
/**
 * Update only the Project/Organization/Environments comment block at the top of .env.setup.
 * Does not touch any KEY=value lines — existing variable values are never erased.
 * Also migrates old REDIS_ACCOUNT_KEY / REDIS_SECRET_KEY to UPSTASH_REDIS_URL when Upstash is enabled.
 */
export function updateEnvSetupHeader(config: SetupConfig): boolean {
  if (!existsSync(ENV_SETUP_PATH)) return false;
  const environmentNames = config.environments.map((environment) => environment.name);
  const newHeaderLines = [
    `${ENV_SETUP_HEADER_PREFIX} (to change these, edit tooling/setup.config.json and re-run pnpm setup:infra:init)`,
    `# Project: ${config.project.displayName} (${config.project.name})`,
    `# Organization: ${config.project.organization}`,
    `# Environments: ${environmentNames.join(', ')}`,
    '',
  ...
  if (config.providers.upstash.enabled) {
    content = migrateRedisToUpstashInContent(content);
  }
  writeFileSync(ENV_SETUP_PATH, content, 'utf-8');
  return true;
}
