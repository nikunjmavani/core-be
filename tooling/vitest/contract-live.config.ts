 ...
 * that provider's credentials are present. Each provider documents its own required variables in
 * its spec header.
 */
export default defineConfig({
  ...
  test: {
    ...
    // Provider specs land one PR at a time on top of this config. With none present yet, "no
    // specs" is the correct state for the tier rather than a failure — and it stays correct for
    // anyone who later removes the last provider spec.
