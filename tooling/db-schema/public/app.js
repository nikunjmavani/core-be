...
const ROW_H = 30, HEAD_H = 44, CARD_W = 268;

/** Distinct palette for module/schema header colors (auth, billing, …). */
const MODULE_PALETTE = [
  '#47c1d6', '#3ecf7c', '#eaa63c', '#a78bfa', '#f0616b',
  '#5b8def', '#e07a5f', '#81b29a', '#c77dff', '#4ecdc4',
];
...
function moduleColor(schema) {
  ...
  const mods = [...new Set((diff?.tables || []).map((t) => t.schema).filter(Boolean).map((s) => String(s).toLowerCase()))].sort();
...
function nodeHTML(t, showDiff) {
  ...
  const schema = t.schema
    ? `<span class="tschema">${escapeHtml(t.schema)}</span>` : '';
...
function renderRail() {
  ...
  for (const v of toShow) {
    ...
    const label = isLatest ? (v.kind === 'latest' || p.migrationMode ? 'Latest' : (v.reason || 'Latest')) : (v.reason || `v${v.id}`);
  ...
  if (more) {
    ...
    if (label) {
      label.textContent = state.versionsOpen
        ? 'Show Latest only'
        : (p.migrationMode ? 'View all migrations' : 'View all versions');
    }
    const nEl = $('#versions-more-n') || more.querySelector('.versions-count');
    if (nEl) {
      if (state.versionsOpen) { nEl.hidden = true; nEl.textContent = ''; }
      else { nEl.hidden = false; nEl.textContent = String(migCount || moreN); }
    }
  }
...
function renderChangelog(diff) {
  ...
  log.querySelectorAll('.cl-item').forEach((it) => it.addEventListener('click', () => {
...
async function refreshMigrationPanel() {
  ...
  try {
    const res = await fetch(`/api/migration?id=${encodeURIComponent(v.id)}`);
    const j = await res.json();
    if (!res.ok) { code.textContent = j.error || 'Failed to load migration file.'; return; }
...
function setupToolbar() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const typing = document.activeElement === $('#palette-q');
    if (e.key === 'Escape') {
      const modal = document.querySelector('.modal.on');
      if (modal) { modal.classList.remove('on'); return; }
      if (state.migOpen) { setMigrationOpen(false); return; }
...
function connect() {
  ...
  es.addEventListener('version', (ev) => {
    ...
    document.title = `${file} · ${nTables} table${nTables === 1 ? '' : 's'} — DB Schema`;
    if (data.diff) {
...
function toSQL(diff) {
  const L = [];
  L.push(`-- generated by DB Schema · ${diff.dialect}`);
...
function setupDeepReview() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const j = await res.json();
      if (j.unavailable) {
        out.innerHTML = `<div class="rv-note">Couldn't find the <code>claude</code> CLI on the server's PATH. Install Claude Code (<code>npm i -g @anthropic-ai/claude-code</code>) or start DB Schema from a shell where <code>claude</code> is available. I've copied a review prompt to your clipboard in the meantime.</div>`;
...
function buildDiagramSVG() {
  ...
  for (const { t, p } of items) {
    ...
    if (t.schema) s += `<text x="36" y="${HEAD_H / 2 - 5}" font-size="9" fill="${C('--ink-faint')}">${escapeHtml(t.schema)}</text>`;
