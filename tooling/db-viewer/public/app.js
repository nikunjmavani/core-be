...
function connect() {
  ...
  es.addEventListener('version', (ev) => {
    ...
    document.title = `${file} · ${nTables} table${nTables === 1 ? '' : 's'} — DB Schema Viewer`;
    if (data.diff) {
...
function toSQL(diff) {
  const L = [];
  L.push(`-- generated by DB Schema Viewer · ${diff.dialect}`);
...
function setupDeepReview() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const j = await res.json();
      if (j.unavailable) {
        out.innerHTML = `<div class="rv-note">Couldn't find the <code>claude</code> CLI on the server's PATH. Install Claude Code (<code>npm i -g @anthropic-ai/claude-code</code>) or start DB Schema Viewer from a shell where <code>claude</code> is available. I've copied a review prompt to your clipboard in the meantime.</div>`;
        try {
