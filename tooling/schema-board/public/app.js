...
const ROW_H = 30, HEAD_H = 40, CARD_W = 236;
...
// ---------------- persistence ----------------
function saveLayout() {
  try { localStorage.setItem('dbd:' + state.key, JSON.stringify(state.pos)); } catch {}
}
function loadLayout() {
  try { return JSON.parse(localStorage.getItem('dbd:' + state.key) || '{}'); } catch { return {}; }
}
function saveTags() {
  try { localStorage.setItem('dbd:tags:' + state.key, JSON.stringify(state.tags || {})); } catch {}
}
function loadTags() {
  try { return JSON.parse(localStorage.getItem('dbd:tags:' + state.key) || '{}'); } catch { return {}; }
}
function saveUIPrefs() {
  try {
    localStorage.setItem('dbd:ui', JSON.stringify({
      diff: state.diffMode,
...
function loadUIPrefs() {
  try { return JSON.parse(localStorage.getItem('dbd:ui') || '{}'); } catch { return {}; }
}

// ---------------- layout ----------------
function cardHeight(t) { return HEAD_H + t.columns.length * ROW_H; }
...
function autoLayout(diff) {
  ...
  const cols = {};
  tables.forEach((t) => { const dp = depth.get(t.name) || 0; (cols[dp] ||= []).push(t); });
  const layerKeys = Object.keys(cols).map(Number).sort((a, b) => a - b);
  ...
  for (let sweep = 0; sweep < 6; sweep++) {
    ...
    for (const k of keys) {
      cols[k] = cols[k].map((t) => {
        ...
        const bary = nb.length ? nb.reduce((s, n) => s + idx.get(n), 0) / nb.length : idx.get(t.name);
        return { t, bary };
      }).sort((a, b) => a.bary - b.bary || a.t.name.localeCompare(b.t.name)).map((o) => o.t);
    }
  }

  const GAP_X = 340, GAP_Y = 40;
  const pos = {};
...
function rectOf(name, diff) {
  const t = diff.tables.find((x) => x.name === name); const p = state.pos[name];
  if (!t || !p) return null;
...
function ensurePositions(diff) {
  ...
  if (Object.keys(state.pos).length === 0) { state.pos = autoLayout(diff); return; }
  ...
  for (const t of missing) {
    ...
    while (guard++ < 300) {
      ...
      for (const other of diff.tables) {
        ...
        if (r && overlaps(box, r)) { cand.y = r.y + r.h + 40; hit = true; break; }
      }
      if (!hit) break;
    }
...
function updateFocusChip() {
  ...
  if (!state.focus) { chip.hidden = true; return; }
  chip.hidden = false;
...
function cycleFocus() {
  ...
  if (!name) { flash('select a table first, then press . to focus on it'); return; }
...
function render() {
  ...
  const changedNames = new Set(diff.tables.filter((t) => t.status && t.status !== 'unchanged').map((t) => t.name));
  ...
  for (const t of diff.tables) {
    ...
    if (!el) { el = document.createElement('div'); el.className = 'node'; world.appendChild(el); nodeEls.set(t.name, el); attachDrag(el, t.name); }
    ...
    const diffDim = showDiff && !onlyChanges && changedNames.size > 0 && (!t.status || t.status === 'unchanged');
    const tag = state.tags && state.tags[t.name];
    el.className = 'node' + (showDiff && t.status ? ' st-' + t.status : '') + (diffDim ? ' diff-dim' : '') + (tag ? ' tagged' : '');
    if (tag) el.style.setProperty('--tagc', tag); else el.style.removeProperty('--tagc');
  ...
  // remove stale nodes
  for (const [name, el] of nodeEls) if (!seen.has(name)) { el.remove(); nodeEls.delete(name); }
...
function nodeHTML(t, showDiff) {
  const badge = showDiff && t.status && t.status !== 'unchanged'
  ...
  const rows = t.columns.map((c) => {
    ...
    if (c.notNull && !c.pk) flags.push('•');
    if (c.unique && !c.pk) flags.push('U');
    const delta = (showDiff && c.status === 'modified' && c.changes)
    ...
    return `<div class="${cls.join(' ')}" data-col="${escapeHtml(c.key)}">
      ...
      <span class="ctype">${escapeHtml(typeLabel(c))}${flags.length ? ' <span class="flag">' + flags.join(' ') + '</span>' : ''}</span>
      ${delta}
    </div>`;
  }).join('');
  return `<div class="head" data-drag="1">
      <span class="chip-notch"></span>
      <span class="tname">${t.schema ? `<span class="tschema">${escapeHtml(t.schema)}</span>.` : ''}${escapeHtml(t.name)}</span>
      ${badge || `<span class="count">${t.columns.length}</span>`}
    </div><div class="rows">${rows}</div>`;
}

// ---------------- edges (traces) ----------------
function colIndex(t, key) { return t.columns.findIndex((c) => c.key === key); }
...
function drawEdges(diff, onlyChanges, changedNames) {
  ...
  for (const r of diff.relations || []) {
    ...
    const pf = state.pos[r.from], pt = state.pos[r.to];
    if (!pf || !pt) continue;
    ...
    path.dataset.from = r.from; path.dataset.to = r.to;
    ...
    hit.dataset.from = r.from; hit.dataset.to = r.to;
  ...
  if (state.diffMode && !state.changesOnly) {
    const changed = new Set(diff.tables.filter((t) => t.status && t.status !== 'unchanged').map((t) => t.name));
    if (changed.size) {
      edgesSvg.querySelectorAll('.edge, .edge-cap, .marker').forEach((e) => {
        if (!changed.has(e.dataset.from) && !changed.has(e.dataset.to)) e.classList.add('diff-faded');
      });
    }
  }
}
...
function drawMarker(x, y, dir, kind, optional, from, to) {
  const add = (d, cls) => {
    ...
    p.setAttribute('d', d); p.setAttribute('class', 'marker' + (cls ? ' ' + cls : ''));
    p.dataset.from = from; p.dataset.to = to; edgesSvg.appendChild(p);
  };
  if (kind === 'many') {
    const apex = x + dir * 13;
    add(`M ${apex} ${y} L ${x} ${y - 7} M ${apex} ${y} L ${x} ${y} M ${apex} ${y} L ${x} ${y + 7}`);
  } else { // one — a single perpendicular bar
    const bx = x + dir * 9;
  ...
  if (optional) {
    ...
    c.setAttribute('cx', cx); c.setAttribute('cy', y); c.setAttribute('r', 3.5);
    c.setAttribute('class', 'marker dot');
    c.dataset.from = from; c.dataset.to = to; edgesSvg.appendChild(c);
  }
}
...
function applyHighlight() {
  ...
  if (path) {
    for (const [name, el] of nodeEls) {
      ...
      if (path.names.includes(name)) el.classList.add(name === path.names[0] || name === path.names[path.names.length - 1] ? 'sel' : 'hi');
      else el.classList.add('dim');
    }
    edgesSvg.querySelectorAll('.edge, .edge-cap, .marker').forEach((e) => {
      e.classList.remove('hl', 'flow', 'faded');
      if (path.pairs.has(e.dataset.from + '→' + e.dataset.to)) { e.classList.add('hl'); if (e.classList.contains('edge')) e.classList.add('flow'); }
  ...
  edgesSvg.querySelectorAll('.edge, .edge-cap, .marker').forEach((e) => {
    e.classList.remove('hl', 'flow', 'faded');
    if (!focus) return;
    if (e.dataset.from === focus || e.dataset.to === focus) { e.classList.add('hl'); if (e.classList.contains('edge')) e.classList.add('flow'); }
    else e.classList.add('faded');
  });
}
...
function findPath(a, b) {
  const rels = (currentDiff().relations || []);
  const adj = {};
  for (const r of rels) { (adj[r.from] ||= []).push(r.to); (adj[r.to] ||= []).push(r.from); }
  const prev = { [a]: null }; const queue = [a];
  while (queue.length) {
    const cur = queue.shift();
    if (cur === b) break;
    for (const n of adj[cur] || []) if (!(n in prev)) { prev[n] = cur; queue.push(n); }
  }
  if (!(b in prev)) return null;
  const names = []; for (let n = b; n != null; n = prev[n]) names.unshift(n);
  return names;
}
...
function pathSQL(names) {
  ...
  for (let i = 1; i < names.length; i++) {
    ...
    if (!r) continue;
    const on = r.from === b ? `${b}.${r.fromColumn} = ${a}.${r.toColumn}` : `${a}.${r.fromColumn} = ${b}.${r.toColumn}`;
    sql += `\nJOIN ${b} ON ${on}`;
  }
  return sql + ';';
}
...
function setPath(a, b) {
  ...
  if (!names) { flash(`no relationship path between ${a} and ${b}`); return; }
  const pairs = new Set();
  for (let i = 1; i < names.length; i++) { pairs.add(names[i - 1] + '→' + names[i]); pairs.add(names[i] + '→' + names[i - 1]); }
  state.path = { names, pairs, sql: pathSQL(names) };
  $('#pb-chain').innerHTML = names.map((n) => `<b>${escapeHtml(n)}</b>`).join('<span class="pb-arr"> → </span>');
...
function setupEdgeTips() {
  ...
  const glowRows = (h, on) => {
    const q = (t, k) => k && document.querySelector(`.node[data-table="${CSS.escape(t)}"] .row[data-col="${CSS.escape(k)}"]`);
  ...
  edgesSvg.addEventListener('pointerover', (e) => {
    const h = e.target.closest('.edge-hit'); if (!h) return;
...
function setupPathBar() {
  ...
  $('#pb-copy').addEventListener('click', async () => {
    try { await navigator.clipboard.writeText(state.path ? state.path.sql : ''); flash('JOIN SQL copied'); } catch {}
  });
}
...
function updateInspector() {
  const el = $('#inspector');
  const diff = currentDiff();
  const close = () => { el.classList.remove('on'); $('#minimap').classList.remove('mm-hide'); };
  if (!state.selected || !diff) { close(); return; }
  ...
  const statusBadge = (state.diffMode && t.status && t.status !== 'unchanged')
    ? `<span class="ins-badge ${t.status}">${t.status}</span>` : '';

  const colHTML = t.columns.map((c) => {
    const cls = ['ins-col']; if (c.pk) cls.push('pk'); if (c.references) cls.push('fk');
    const flags = [];
    if (c.pk) flags.push('PK'); if (c.references && !c.pk) flags.push('FK');
    if (c.notNull && !c.pk) flags.push('NOT NULL'); if (c.unique && !c.pk) flags.push('UNIQUE');
    if (c.default != null) flags.push('= ' + c.default);
    const enumLine = c.isEnum && c.enumValues
      ? `<div class="ins-enum">enum <span class="ev">${c.enumValues.map(escapeHtml).join(' · ')}</span></div>` : '';
    return `<div class="${cls.join(' ')}"><span class="dotp"></span><span class="cn">${escapeHtml(c.name)}</span><span class="ct">${escapeHtml(typeLabel(c))}</span></div>
      ${flags.length ? `<div class="cf" style="padding:0 0 3px 13px">${escapeHtml(flags.join(' · '))}</div>` : ''}${enumLine}`;
  }).join('');

  const outHTML = out.length ? out.map((r) =>
    `<div class="ins-rel" data-goto="${escapeHtml(r.to)}"><span class="cn">${escapeHtml(r.fromColumn)}</span><span class="arw">→</span><b>${escapeHtml(r.to)}</b>.${escapeHtml(r.toColumn)}${r.onDelete ? `<span class="od">on delete ${escapeHtml(r.onDelete)}</span>` : ''}</div>`
  ).join('') : '<div class="ins-empty">none</div>';

  const incHTML = inc.length ? inc.map((r) =>
    `<div class="ins-rel" data-goto="${escapeHtml(r.from)}"><b>${escapeHtml(r.from)}</b>.${escapeHtml(r.fromColumn)}<span class="arw">→</span><span class="cn">${escapeHtml(r.toColumn)}</span></div>`
  ).join('') : '<div class="ins-empty">none</div>';

  const idxHTML = (t.indexes && t.indexes.length)
    ? t.indexes.map((i) => `<div class="ins-rel"><span class="cn">${escapeHtml(i.name)}</span><span class="cf">${i.unique ? 'unique · ' : ''}${escapeHtml((i.columns || []).join(', '))}</span></div>`).join('')
    : '';

  const TAG_COLORS = ['#47c1d6', '#3ecf7c', '#eaa63c', '#a78bfa', '#f0616b'];
  const curTag = (state.tags || {})[t.name] || '';
  const tagHTML = `<button class="tag-sw none${curTag === '' ? ' on' : ''}" data-c="" title="No tag">∅</button>` +
    TAG_COLORS.map((c) => `<button class="tag-sw${curTag === c ? ' on' : ''}" data-c="${c}" style="--c:${c}" title="Tag ${c}"></button>`).join('');

  el.innerHTML = `
    <div class="ins-head">
      <span class="ins-title">${escapeHtml(t.name)}</span>${statusBadge}
      <span class="ins-sub">${t.dialect || diff.dialect} · ${t.columns.length} cols</span>
      <button class="ins-close" title="Close (Esc)">✕</button>
    </div>
    <div class="ins-body">
      <div class="ins-sec"><h5>Color tag <span class="n"></span></h5><div class="tag-row">${tagHTML}</div></div>
  ...
  el.querySelectorAll('.tag-sw').forEach((b) => b.addEventListener('click', () => {
    state.tags = state.tags || {};
    if (b.dataset.c) state.tags[t.name] = b.dataset.c; else delete state.tags[t.name];
    saveTags(); render();
  }));
  el.querySelector('.ins-close').addEventListener('click', () => setSelected(null));
  el.querySelectorAll('[data-goto]').forEach((r) => r.addEventListener('click', () => {
    const name = r.dataset.goto; if (!state.pos[name]) return;
    state.selected = name; applyHighlight(); updateInspector(); centerOn(name);
  }));
}
...
function zoomAt(cx, cy, factor) {
  ...
  let k = Math.min(2.4, Math.max(0.2, k0 * factor));
  const r = k / k0;
...
function setupCanvas() {
  const stage = $('#stage');
  const rubber = $('#rubber');
  let mode = null, sx = 0, sy = 0, ox = 0, oy = 0; // 'pan' | 'rubber'
  ...
  stage.addEventListener('pointerdown', (e) => {
    ...
    if (e.shiftKey) {
      mode = 'rubber';
      rubber.hidden = false;
      rubber.style.left = (sx - r.left) + 'px'; rubber.style.top = (sy - r.top) + 'px';
      rubber.style.width = '0px'; rubber.style.height = '0px';
    } else {
      mode = 'pan'; stage.classList.add('panning');
      clearMulti();
  ...
  const endPan = (e) => {
    if (mode === 'rubber') {
      ...
      const r = stage.getBoundingClientRect();
      const { x, y, k } = state.view;
      const wx1 = (Math.min(sx, e.clientX) - r.left - x) / k, wy1 = (Math.min(sy, e.clientY) - r.top - y) / k;
      const wx2 = (Math.max(sx, e.clientX) - r.left - x) / k, wy2 = (Math.max(sy, e.clientY) - r.top - y) / k;
      const diff = currentDiff();
      const hits = new Set();
      if (diff) for (const t of diff.tables) {
        const p = state.pos[t.name]; if (!p || !nodeEls.has(t.name)) continue;
        const h = cardHeight(t);
        if (p.x < wx2 && p.x + CARD_W > wx1 && p.y < wy2 && p.y + h > wy1) hits.add(t.name);
      }
      state.multi = hits.size ? hits : null;
      applyMulti();
      if (hits.size) flash(`${hits.size} table${hits.size > 1 ? 's' : ''} selected — drag any one to move the group`);
    }
    mode = null; stage.classList.remove('panning');
  };
...
function attachDrag(el, name) {
  el.addEventListener('pointerdown', (e) => {
    ...
    const move = (ev) => {
      ...
      for (const n of group) {
        ...
        if (nel) { nel.style.left = state.pos[n].x + 'px'; nel.style.top = state.pos[n].y + 'px'; }
      }
    ...
    const up = () => {
      ...
      drawEdges(currentDiff(), state.diffMode && state.changesOnly, null);
      saveLayout(); renderMinimap(currentDiff());
    };
  ...
  // hover + select
  el.addEventListener('pointerenter', () => { state.hover = name; applyHighlight(); });
  el.addEventListener('pointerleave', () => { if (state.hover === name) state.hover = null; applyHighlight(); });
  el.addEventListener('click', (e) => {
    if (e.shiftKey && state.selected && state.selected !== name) { setPath(state.selected, name); return; }
...
function animateView(tx, ty, tk, dur = 420) {
  cancelViewAnim();
  if (prefersReduce) { state.view.x = tx; state.view.y = ty; state.view.k = tk; applyTransform(); return; }
...
function fitWhenReady(tries = 30) {
  const r = $('#stage').getBoundingClientRect();
  if (r.width > 200 && r.height > 160 && nodeEls.size) { fitView(false); return; }
  if (tries > 0) requestAnimationFrame(() => fitWhenReady(tries - 1));
}

function fitView(animate = true) {
  const diff = currentDiff(); if (!diff) return;
...
function fitToNames(names, diff, animate = true) {
  ...
  for (const name of names) {
    ...
    maxX = Math.max(maxX, p.x + CARD_W); maxY = Math.max(maxY, p.y + cardHeight(t));
    count++;
  }
  ...
  const k = Math.max(0.28, Math.min(kMax, Math.min((rect.width - pad * 2) / bw, (rect.height - pad * 2) / bh)));
...
function fitToChanges(diff, animate = true) {
  ...
  if (!names.length) { fitView(animate); return; }
  fitToNames(names, diff, animate);
}

// ---------------- minimap ----------------
function updateViewport() {
  const diff = currentDiff(); if (diff) renderMinimap(diff);
}
...
function minimapTransform(diff) {
  ...
  if (names.length) {
    ...
    for (const name of names) {
      const t = diff.tables.find((x) => x.name === name); const p = state.pos[name]; if (!t || !p) continue;
      minX = Math.min(minX, p.x); minY = Math.min(minY, p.y);
      maxX = Math.max(maxX, p.x + CARD_W); maxY = Math.max(maxY, p.y + cardHeight(t));
    }
  }
  const W = 172, H = 116, pad = 8;
...
function setupMinimapNav() {
  ...
  mm.addEventListener('pointerdown', (e) => { navigating = true; jump(e); try { mm.setPointerCapture(e.pointerId); } catch {} });
  mm.addEventListener('pointermove', (e) => { if (navigating) jump(e); });
...
function renderMinimap(diff) {
  ...
  for (const name of names) {
    ...
    const tagFill = (!cls && state.tags && state.tags[t.name]) ? ` style="fill:${state.tags[t.name]}"` : '';
...
function renderRail() {
  ...
  if (sec) {
    ...
    const tog = $('#versions-toggle');
    if (tog) {
      tog.setAttribute('aria-expanded', state.versionsOpen ? 'true' : 'false');
      tog.title = state.versionsOpen ? 'Collapse versions — show Latest only' : 'Expand all versions';
    }
    const countEl = $('#versions-count');
    if (countEl) {
      countEl.textContent = state.versionsOpen
  ...
  for (const v of toShow) {
    ...
    let badge = '';
    if (v.kind === 'latest' || (v.id === p.current && v.kind !== 'migration')) badge = '<span class="vbadge l">Latest</span>';
    ...
    else if (v.kind === 'migration' && v.structural === false) badge = '<span class="vbadge n">no schema</span>';
    ...
    const label = isLatest ? (v.kind === 'latest' || p.migrationMode ? 'Latest (Drizzle)' : (v.reason || 'Latest')) : (v.reason || `v${v.id}`);
    ...
    item.addEventListener('click', (e) => {
      if (e.metaKey || e.ctrlKey) { state.base = state.base === v.id ? null : v.id; loadComparison(); }
      else {
        state.target = v.id; state.liveTarget = (v.id === p.current);
  ...
  if (state.base && state.target) {
    ...
    cl.querySelector('.cmp-clear').addEventListener('click', () => { state.base = null; loadComparison(); });
  } else {
    const toV = p.versions.find((x) => x.id === state.target);
    ...
    if (p.migrationMode && toV) {
      ...
      cl.textContent = state.target && state.target > 1 ? `v${state.target - 1} → v${state.target}` : 'baseline';
    }
  }
}
...
function renderChangelog(diff) {
  const sum = $('#summary'); const log = $('#changelog');
  const setN = (n) => { $('#tab-changes-n').textContent = n ? n : ''; };
  if (!diff || !diff.changelog) { setN(0); sum.innerHTML = ''; log.innerHTML = '<div class="empty">No prior version to compare.</div>'; return; }
  setN(diff.changelog.length);
  const c = diff.counts || {};
  const pills = [];
  if (c.tablesAdded) pills.push(`<span class="pill p-add">+${c.tablesAdded} table${c.tablesAdded > 1 ? 's' : ''}</span>`);
  if (c.tablesRemoved) pills.push(`<span class="pill p-rem">−${c.tablesRemoved} table${c.tablesRemoved > 1 ? 's' : ''}</span>`);
  if (c.columnsAdded) pills.push(`<span class="pill p-add">+${c.columnsAdded} column${c.columnsAdded > 1 ? 's' : ''}</span>`);
  if (c.columnsModified) pills.push(`<span class="pill p-mod">~${c.columnsModified} changed</span>`);
  if (c.columnsRemoved) pills.push(`<span class="pill p-rem">−${c.columnsRemoved} column${c.columnsRemoved > 1 ? 's' : ''}</span>`);
  if (c.enumsChanged) pills.push(`<span class="pill p-mod">~${c.enumsChanged} enum${c.enumsChanged > 1 ? 's' : ''}</span>`);
  ...
  const verb = (k) => ({ 'table-added': 'new table', 'table-removed': 'dropped table', 'column-added': 'added', 'column-removed': 'removed', 'column-modified': 'changed', 'enum-added': 'new enum', 'enum-removed': 'dropped enum', 'enum-modified': 'enum changed' }[k] || k);
  log.innerHTML = diff.changelog.map((e) => {
    const where = e.enum ? `enum ${e.enum}` : (e.column ? `${e.table}.${e.column}` : e.table);
    const loc = e.enum ? `<b>${escapeHtml(e.enum)}</b> <span class="cl-kind">enum</span>`
      : (e.column ? `<b>${escapeHtml(e.table)}</b>.${escapeHtml(e.column)}` : `<b>${escapeHtml(e.table)}</b>`);
    const note = `${verb(e.kind)} ${where}${e.detail ? ' (' + e.detail + ')' : ''}`;
    return `<div class="cl-item" data-table="${escapeHtml(e.table || '')}">
      ...
      <span class="cl-detail">${escapeHtml(e.detail || '')}</span>
      <button class="cl-add" data-note="Re: ${escapeHtml(note)} — " title="Comment on this change to the agent">＋</button>
    </div>`;
  }).join('');
  log.querySelectorAll('.cl-item').forEach((it) => it.addEventListener('click', (ev) => {
    if (ev.target.closest('.cl-add')) return;
    const name = it.dataset.table; if (!state.pos[name]) return;
    state.selected = name; if (!state.diffMode) toggleDiff(true); applyHighlight(); updateInspector(); centerOn(name);
  }));
  log.querySelectorAll('.cl-add').forEach((b) => b.addEventListener('click', (ev) => {
    ev.stopPropagation(); addFeedback(b.dataset.note); switchPanel('feedback'); $('#feedback-text').focus();
  }));
}
...
function navigateTable(dir) {
  const diff = currentDiff(); if (!diff) return;
  const centers = {};
  for (const t of diff.tables) { const p = state.pos[t.name]; if (p) centers[t.name] = { x: p.x + CARD_W / 2, y: p.y + cardHeight(t) / 2 }; }
  const names = Object.keys(centers); if (!names.length) return;
  const cur = state.selected && centers[state.selected] ? state.selected : null;
  if (!cur) { // nothing selected → start from the top-left table
    selectAndFocus(names.sort((a, b) => centers[a].y - centers[b].y || centers[a].x - centers[b].x)[0]);
    return;
  }
  const c = centers[cur];
  ...
  for (const n of names) {
    if (n === cur) continue;
    const dx = centers[n].x - c.x, dy = centers[n].y - c.y;
...
function centerOn(name) {
  const diff = currentDiff(); const t = diff.tables.find((x) => x.name === name); const p = state.pos[name];
  if (!t || !p) return;
  const stage = $('#stage').getBoundingClientRect();
...
async function loadComparison(opts = {}) {
  ...
  if (state.liveTarget || (!state.base && state.target === p.current)) {
    ...
    try {
      const res = await fetch(`/api/diff?from=${from}&to=${state.target}`);
      const j = await res.json();
      state._diff = j.diff;
    } catch { state._diff = p.diff; }
  }
  ...
  if (diff) {
    ...
    $('#stats').textContent = `${nTables} tables · ${nRels} FK${nRels === 1 ? '' : 's'}${nEnums ? ` · ${nEnums} enum${nEnums === 1 ? '' : 's'}` : ''} · ${diff.dialect || 'postgres'}`;
  }
  render();
  renderRail();
...
function ensurePositionsFor(names) {
  ...
  for (const name of names) {
    ...
    if (!keys.length) { state.pos[name] = { x: 40, y: 40 }; placed = true; continue; }
    let sx = 0, sy = 0;
    for (const k of keys) { sx += state.pos[k].x; sy += state.pos[k].y; }
...
function toggleDiff(force) {
  ...
  if (!state.diffMode) { state.changesOnly = false; $('#btn-only').classList.remove('on'); }
...
function setupToolbar() {
  ...
  $('#zoom-in').addEventListener('click', () => { const r = $('#stage').getBoundingClientRect(); zoomAt(r.width / 2, r.height / 2, 1.2); });
  $('#zoom-out').addEventListener('click', () => { const r = $('#stage').getBoundingClientRect(); zoomAt(r.width / 2, r.height / 2, 1 / 1.2); });
  $('#fit').addEventListener('click', fitView);
  $('#relayout').addEventListener('click', () => { pushLayout(); state.pos = autoLayout(currentDiff()); saveLayout(); render(); fitView(); });
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const typing = document.activeElement === $('#feedback-text') || document.activeElement === $('#palette-q');
    if (e.key === 'Escape') {
      const modal = document.querySelector('.modal.on');
      if (modal) { modal.classList.remove('on'); return; }
      if (state.path) { clearPath(); return; }
      if (state.multi) { clearMulti(); return; }
      if (state.focus) { state.focus = null; updateFocusChip(); render(); fitView(); return; }
      state.selected = null; updateInspector(); applyHighlight(); return;
    }
    if (typing) return;
    if ((e.metaKey || e.ctrlKey) && (e.key === 'z' || e.key === 'Z')) { e.preventDefault(); undoLayout(); return; }
    if (e.key === '.') { e.preventDefault(); cycleFocus(); }
    else if (e.key === '/') { e.preventDefault(); openPalette(); }
    else if (e.key === ' ' && tm.on) { e.preventDefault(); tmPlay(); }
    else if (e.key === '?') { e.preventDefault(); $('#help').classList.toggle('on'); }
    else if (e.key === 'f' || e.key === 'F') fitView();
    else if (e.key === 'd' || e.key === 'D') toggleDiff();
    else if (e.key === '+' || e.key === '=') { const r = $('#stage').getBoundingClientRect(); zoomAt(r.width / 2, r.height / 2, 1.2); }
    else if (e.key === '-' || e.key === '_') { const r = $('#stage').getBoundingClientRect(); zoomAt(r.width / 2, r.height / 2, 1 / 1.2); }
    else {
      const arrow = { ArrowUp: 'up', ArrowDown: 'down', ArrowLeft: 'left', ArrowRight: 'right' }[e.key];
      if (arrow) { e.preventDefault(); navigateTable(arrow); }
    }
  });
}
...
function connect() {
  ...
  es.addEventListener('version', (ev) => {
    ...
    if (!state.tags) state.tags = loadTags();
    const pathLabel = data.projectName
      ? (data.migrationsDir ? `${data.projectName} · ${data.migrationsDir}` : data.projectName)
      : (data.target || '');
    ...
    document.title = `${file} · ${nTables} table${nTables === 1 ? '' : 's'} — schema·board`;
    if (data.diff) {
      const nRels = (data.diff.relations || []).length, nEnums = (data.diff.enums || []).length;
      $('#stats').textContent = `${nTables} tables · ${nRels} FK${nRels === 1 ? '' : 's'}${nEnums ? ` · ${nEnums} enum${nEnums === 1 ? '' : 's'}` : ''} · ${data.diff.dialect}`;
    }
    loadComparison();
    if (isNew) {
...
// ---------------- export (SQL / DBML) ----------------
const SQL_TYPES = {
  uuid: 'uuid', varchar: 'varchar', text: 'text', char: 'char',
  integer: 'integer', int: 'integer', smallint: 'smallint', bigint: 'bigint',
  serial: 'serial', bigserial: 'bigserial', boolean: 'boolean', real: 'real',
  doublePrecision: 'double precision', numeric: 'numeric', decimal: 'numeric',
  timestamp: 'timestamp', date: 'date', time: 'time', jsonb: 'jsonb', json: 'json',
};
function sqlType(c) {
...
function columnDefSQL(c, { withPk = true } = {}) {
  ...
  if (c.references) s += ` REFERENCES ${c.references.table}(${c.references.column})${c.references.onDelete ? ' ON DELETE ' + c.references.onDelete.toUpperCase() : ''}`;
  return s;
}
function createTableSQL(t) {
  ...
  L.push(lines.join(',\n'));
  L.push(');');
  for (const i of t.indexes || []) L.push(`CREATE ${i.unique ? 'UNIQUE ' : ''}INDEX ${i.name} ON ${t.name} (${(i.columns || []).join(', ')});`);
  return L.join('\n');
}

function toSQL(diff) {
  const L = [];
  L.push(`-- generated by schema·board · ${diff.dialect}`);
  for (const e of diff.enums || []) L.push(`CREATE TYPE ${e.name} AS ENUM (${e.values.map((v) => `'${v}'`).join(', ')});`);
...
function toMigration(diff) {
  ...
  const up = [];
  const label = state.base && state.target ? `v${state.base} → v${state.target}`
    : (state.target > 1 ? `v${state.target - 1} → v${state.target}` : 'initial');
  up.push(`-- migration · ${label} · ${diff.dialect}`, '');

  // 1) enums first (types must exist before columns use them)
  for (const e of cl) {
    if (e.kind === 'enum-added') up.push(`CREATE TYPE ${e.enum} AS ENUM (${(e.values || []).map((v) => `'${v}'`).join(', ')});`);
    if (e.kind === 'enum-modified') {
      for (const v of e.added || []) up.push(`ALTER TYPE ${e.enum} ADD VALUE '${v}';   -- note: cannot run inside a transaction block`);
      for (const v of e.removed || []) up.push(`-- Postgres can't drop enum value '${v}' from ${e.enum} directly; recreate the type if needed.`);
    }
  ...
  for (const t of diff.tables) {
    ...
    for (const c of t.columns) {
      if (c.status === 'added') stmts.push(`ALTER TABLE ${t.name} ADD COLUMN ${columnDefSQL(c, { withPk: false })};`);
        ...
        if ('notNull' in d) stmts.push(`ALTER TABLE ${t.name} ALTER COLUMN ${c.name} ${d.notNull ? 'SET NOT NULL' : 'DROP NOT NULL'};`);
        if ('default' in d) { const def = sqlDefault(c); stmts.push(`ALTER TABLE ${t.name} ALTER COLUMN ${c.name} ${def ? 'SET DEFAULT ' + def : 'DROP DEFAULT'};`); }
        if ('unique' in d) stmts.push(d.unique ? `CREATE UNIQUE INDEX ${t.name}_${c.name}_key ON ${t.name} (${c.name});` : `-- drop the unique constraint/index on ${t.name}.${c.name}`);
        if ('references' in d) stmts.push(d.references ? `ALTER TABLE ${t.name} ADD CONSTRAINT ${t.name}_${c.name}_fkey FOREIGN KEY (${c.name}) REFERENCES ${d.references.table}(${d.references.column})${d.references.onDelete ? ' ON DELETE ' + d.references.onDelete.toUpperCase() : ''};` : `-- drop the foreign key on ${t.name}.${c.name}`);
      }
    }
...
function toDBML(diff) {
  ...
  for (const t of diff.tables) {
    ...
    for (const c of t.columns) {
      ...
      if (c.references) refs.push(`Ref: ${t.name}.${c.name} > ${c.references.table}.${c.references.column}`);
    }
    L.push('}\n');
  }
...
function openExport() {
  const diff = currentDiff(); if (!diff) return;
  const modal = $('#export'); modal.classList.add('on');
  renderExport();
}
// lightweight syntax highlighting: comments, strings, keywords (escaped first — safe)
const SQL_KEYWORDS = /\b(CREATE|TABLE|TYPE|AS|ENUM|PRIMARY KEY|NOT NULL|UNIQUE|DEFAULT|REFERENCES|ON DELETE|CASCADE|SET NULL|ALTER|ADD|DROP|COLUMN|CONSTRAINT|FOREIGN KEY|INDEX|ON|SET|VALUE|Table|Enum|Ref)\b/g;
...
function renderExport() {
  const diff = currentDiff(); if (!diff) return;
  const code = exportTab === 'migration' ? toMigration(diff) : exportTab === 'dbml' ? toDBML(diff) : toSQL(diff);
  const el = $('#export-code');
  el.innerHTML = highlightCode(code);
  el.dataset.raw = code; // copy/download read the raw text
  document.querySelectorAll('#export .tab').forEach((b) => b.classList.toggle('on', b.dataset.tab === exportTab));
  $('#export-sub').textContent = exportTab === 'migration'
...
function setupExport() {
  ...
  $('#export').addEventListener('click', (e) => { if (e.target.id === 'export') $('#export').classList.remove('on'); });
  document.querySelectorAll('#export .tab').forEach((b) => b.addEventListener('click', () => { exportTab = b.dataset.tab; renderExport(); }));
  $('#export-copy').addEventListener('click', async () => {
    try { await navigator.clipboard.writeText($('#export-code').textContent); flash('copied to clipboard'); } catch {}
  });
  $('#export-download').addEventListener('click', () => {
    const name = exportTab === 'dbml' ? 'schema.dbml' : exportTab === 'migration' ? 'migration.sql' : 'schema.sql';
...
function analyzeSchema(diff) {
  ...
  for (const t of (diff.tables || [])) {
    ...
    (t.indexes || []).forEach((i) => { if (i.columns && i.columns[0]) indexed.add(i.columns[0]); });

    if (!pk.length) add('error', t.name, null, 'No primary key.', `Add a primary key to "${t.name}".`);

    for (const c of cols) {
      if (c.references && !c.pk && !indexed.has(c.key))
        add('warn', t.name, c.name, 'Foreign key without an index — joins & cascade deletes do full scans.', `Add an index on ${t.name}.${c.name}.`);
      if (/(price|amount|total|cost|balance|fee|salary|revenue|discount)/i.test(c.name) && /^(real|double|doublePrecision|float)$/i.test(c.type))
        add('warn', t.name, c.name, 'Monetary value stored as floating point — rounding errors.', `Store ${t.name}.${c.name} as integer (cents) or numeric.`);
      if (/^varchar$/i.test(c.type) && !c.length)
        add('info', t.name, c.name, 'varchar with no length.', `Set a length on ${t.name}.${c.name}, or use text.`);
      if (/^(email|slug|username|sku|handle|phone_e164)$/i.test(c.name) && !c.unique && !c.pk)
        add('warn', t.name, c.name, `"${c.name}" looks like a natural key but isn't unique.`, `Add a unique constraint on ${t.name}.${c.name}.`);
      if (/^boolean$/i.test(c.type) && !c.notNull)
        add('info', t.name, c.name, 'Nullable boolean (three-valued logic).', `Make ${t.name}.${c.name} NOT NULL with a default.`);
      if (c.isEnum && !c.notNull && c.default == null)
        add('info', t.name, c.name, 'Enum column is nullable with no default.', `Give ${t.name}.${c.name} a default or make it NOT NULL.`);
      if (/^timestamp$/i.test(c.type) && !c.withTimezone)
        add('info', t.name, c.name, 'timestamp without time zone.', `Use { withTimezone: true } (timestamptz) on ${t.name}.${c.name}.`);
      if (/[A-Z]/.test(c.name))
        add('info', t.name, c.name, "Column name isn't snake_case.", `Rename ${t.name}.${c.name} to snake_case.`);
    }
    if (names.has('created_at') && !names.has('updated_at'))
      add('info', t.name, null, 'Has created_at but no updated_at.', `Add an updated_at column to "${t.name}".`);
    if (![...names].some((n) => /(created_at|updated_at|inserted_at)/.test(n)))
      add('info', t.name, null, 'No audit timestamps.', `Consider created_at / updated_at on "${t.name}".`);
  }
...
function renderSuggestions(diff) {
  ...
  if (!findings.length) { list.innerHTML = '<div class="empty">No suggestions — schema looks clean. ✓</div>'; return; }
  ...
  list.innerHTML = findings.map((f, i) => `
    <div class="sg" data-table="${escapeHtml(f.table)}">
      ...
      <button class="sg-add" data-i="${i}" title="Add to feedback for the agent">＋</button>
    </div>`).join('');
  list.querySelectorAll('.sg-add').forEach((b) => b.addEventListener('click', (e) => {
    e.stopPropagation();
    const f = findings[Number(b.dataset.i)];
    addFeedback(f.fix);
  }));
  list.querySelectorAll('.sg').forEach((el) => el.addEventListener('click', () => {
    const name = el.dataset.table; if (state.pos[name]) { state.selected = name; applyHighlight(); updateInspector(); centerOn(name); }
  }));
}

// ---------------- feedback for the agent ----------------
function addFeedback(line) {
  const ta = $('#feedback-text');
  const existing = ta.value.trim();
  ta.value = (existing ? existing + '\n' : '') + '- ' + line;
  updateFeedbackCount();
  flash('added to feedback');
  // nudge the user toward the tab without forcing it
  $('#tab-feedback-n').classList.add('pulse');
  setTimeout(() => $('#tab-feedback-n').classList.remove('pulse'), 1200);
}
function updateFeedbackCount() {
  const n = $('#feedback-text').value.split('\n').filter((l) => l.trim()).length;
  $('#tab-feedback-n').textContent = n ? n : '';
}
function feedbackPrompt() {
  const p = state.payload || {};
  const body = $('#feedback-text').value.trim();
  const ctx = [];
  ctx.push(`I'm reviewing my Drizzle schema in schema·board.`);
  if (p.target) ctx.push(`Schema file: ${p.target}`);
  if (state.base && state.target) ctx.push(`Looking at the diff v${state.base} → v${state.target}.`);
  else if (state.target > 1) ctx.push(`Looking at the latest change (v${state.target - 1} → v${state.target}).`);
  return `${ctx.join(' ')}\n\nPlease make these changes to the schema:\n\n${body}\n\nKeep the changes minimal and update the Drizzle schema file(s) accordingly.`;
}
function setupFeedback() {
  const ta = $('#feedback-text');
  ta.addEventListener('input', updateFeedbackCount);
  $('#fb-copy').addEventListener('click', async () => {
    if (!ta.value.trim()) { flash('nothing to copy yet'); return; }
    try { await navigator.clipboard.writeText(feedbackPrompt()); flash('prompt copied — paste to your agent'); } catch {}
  });
  $('#fb-clear').addEventListener('click', () => { ta.value = ''; updateFeedbackCount(); });
  document.querySelectorAll('.stab').forEach((b) => b.addEventListener('click', () => switchPanel(b.dataset.panel)));
}
function switchPanel(name) {
  document.querySelectorAll('.stab').forEach((b) => b.classList.toggle('on', b.dataset.panel === name));
  document.querySelectorAll('.panel').forEach((p) => p.classList.toggle('hidden', p.id !== 'panel-' + name));
}

// tiny, safe markdown → HTML (escape first, then a limited subset)
...
function setupDeepReview() {
  ...
  btn.addEventListener('click', async () => {
    ...
    out.classList.add('on'); out.innerHTML = '<div class="rv-loading">Asking Claude to review your schema…</div>';
    try {
      const res = await fetch('/api/review', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sql: toSQL(diff) }) });
      const j = await res.json();
      if (j.unavailable) {
        out.innerHTML = `<div class="rv-note">Couldn't find the <code>claude</code> CLI on the server's PATH. Install Claude Code (<code>npm i -g @anthropic-ai/claude-code</code>) or start schema·board from a shell where <code>claude</code> is available. I've copied a review prompt to your clipboard in the meantime.</div>`;
        try { await navigator.clipboard.writeText(`Please review this Postgres/Drizzle schema as a senior DB engineer — correctness, indexing, integrity, naming, footguns:\n\n\`\`\`sql\n${toSQL(diff)}\n\`\`\``); flash('review prompt copied'); } catch {}
      } else if (j.review) {
...
function applyThemePref(pref) {
  ...
  try { localStorage.setItem('dbd:theme', pref); } catch {}
...
function setupTheme() {
  let pref = 'system';
  try { pref = localStorage.getItem('dbd:theme') || 'system'; } catch {}
  applyThemePref(pref);
...
function buildDiagramSVG() {
  const diff = currentDiff(); if (!diff || !nodeEls.size) return null;
  ...
  const PAD = 48, W = Math.ceil(maxX - minX + PAD * 2), H = Math.ceil(maxY - minY + PAD * 2);
  ...
  edgesSvg.querySelectorAll('.edge').forEach((p) => { s += `<path d="${p.getAttribute('d')}" stroke="${C('--trace')}" stroke-width="1.6" opacity="0.55" fill="none"/>`; });
  edgesSvg.querySelectorAll('.marker').forEach((m) => {
    if (m.tagName === 'path') s += `<path d="${m.getAttribute('d')}" stroke="${C('--trace')}" stroke-width="1.6" opacity="0.55" fill="none"/>`;
    else s += `<circle cx="${m.getAttribute('cx')}" cy="${m.getAttribute('cy')}" r="${m.getAttribute('r')}" stroke="${C('--trace')}" fill="${C('--board')}" opacity="0.8"/>`;
  });
  edgesSvg.querySelectorAll('.edge-cap').forEach((c) => { s += `<circle cx="${c.getAttribute('cx')}" cy="${c.getAttribute('cy')}" r="3" fill="${C('--trace')}" opacity="0.7"/>`; });
  ...
  for (const { t, p } of items) {
    ...
    s += `<rect x="12" y="${HEAD_H / 2 - 3.5}" width="7" height="7" rx="2" fill="${(state.tags && state.tags[t.name]) || C('--trace')}"/>`;
    s += `<text x="26" y="${HEAD_H / 2 + 4.5}" font-size="13" font-weight="600" fill="${C('--ink')}">${escapeHtml(t.name)}</text>`;
    const headHint = showDiff && t.status && t.status !== 'unchanged' ? t.status.toUpperCase() : String(t.columns.length);
    s += `<text x="${CARD_W - 12}" y="${HEAD_H / 2 + 4}" font-size="9.5" font-weight="700" text-anchor="end" fill="${showDiff && stC[t.status] ? stC[t.status] : C('--ink-faint')}">${headHint}</text>`;
    t.columns.forEach((c, i) => {
      ...
      if (i > 0) s += `<line x1="0" y1="${rowY}" x2="${CARD_W}" y2="${rowY}" stroke="${C('--hair-soft')}"/>`;
      const pin = c.pk ? C('--gold') : (c.references ? C('--trace') : C('--hair'));
      s += `<circle cx="15.5" cy="${y}" r="3.5" fill="${pin}"/>`;
      const nameFill = showDiff && c.status && c.status !== 'unchanged' ? stC[c.status] : (c.pk ? C('--gold') : C('--ink'));
...
async function exportDiagramPNG() {
  ...
  if (!built) { flash('nothing to export'); return; }
  ...
  try {
    ...
    cv.width = built.W * scale; cv.height = built.H * scale;
    cv.getContext('2d').drawImage(img, 0, 0, cv.width, cv.height);
    cv.toBlob((b) => {
      const a = document.createElement('a');
      a.href = URL.createObjectURL(b); a.download = 'schema.png'; a.click();
      URL.revokeObjectURL(a.href);
...
function tmVersions() { return (state.payload && state.payload.versions) || []; }

function tmGo(idx) {
  const vs = tmVersions(); if (!vs.length) return;
  ...
  state.target = v.id; state.base = null;
  state.liveTarget = (v.id === state.payload.current);
  if (!state.diffMode) toggleDiff(true);
  loadComparison();
}
function tmIndex() { return Number($('#tm-slider').value); }

function tmStop() {
  tm.playing = false; clearInterval(tm.timer); tm.timer = null;
  $('#tm-play').textContent = '▶';
}
function tmPlay() {
  const vs = tmVersions(); if (vs.length < 2) return;
  if (tm.playing) { tmStop(); return; }
  tm.playing = true; $('#tm-play').textContent = '⏸';
  if (tmIndex() >= vs.length - 1) tmGo(0);          // restart from the beginning
  tm.timer = setInterval(() => {
    const i = tmIndex();
    if (i >= tmVersions().length - 1) { tmStop(); return; }
    tmGo(i + 1);
  }, 1700);
}
...
function openTimeMachine() {
  const vs = tmVersions();
  if (vs.length < 2) { flash('only one version yet — edit the schema to build history'); return; }
  tm.on = true;
  const slider = $('#tm-slider');
  slider.min = 0; slider.max = vs.length - 1;
  $('#tm-ticks').innerHTML = vs.map((_, i) =>
    `<i style="left:${vs.length > 1 ? (i / (vs.length - 1)) * 100 : 0}%"></i>`).join('');
...
function closeTimeMachine() {
  ...
  const vs = tmVersions();
  if (vs.length) { state.target = state.payload.current; state.base = null; state.liveTarget = true; loadComparison(); }
}
function setupTimeMachine() {
  ...
  $('#tm-prev').addEventListener('click', () => { tmStop(); tmGo(tmIndex() - 1); });
  $('#tm-next').addEventListener('click', () => { tmStop(); tmGo(tmIndex() + 1); });
  $('#tm-close').addEventListener('click', closeTimeMachine);
  $('#tm-slider').addEventListener('input', () => { tmStop(); tmGo(tmIndex()); });
}
...
function paletteCommands() {
  ...
  c.push({ group: 'Actions', ico: '⧉', label: 'Show only changes', run: () => { if (!state.diffMode) toggleDiff(true); if (!state.changesOnly) toggleOnly(); } });
  c.push({ group: 'Actions', ico: '⤢', label: 'Fit to screen', hint: 'F', run: () => fitView() });
  c.push({ group: 'Actions', ico: '✦', label: 'Auto-arrange tables', run: () => { state.pos = autoLayout(currentDiff()); saveLayout(); render(); fitView(); } });
  c.push({ group: 'Actions', ico: '↧', label: 'Export SQL', run: () => { exportTab = 'sql'; openExport(); } });
  c.push({ group: 'Actions', ico: '↧', label: 'Export DBML', run: () => { exportTab = 'dbml'; openExport(); } });
  c.push({ group: 'Actions', ico: '↧', label: 'Export migration (ALTER)', run: () => { exportTab = 'migration'; openExport(); } });
  c.push({ group: 'Actions', ico: '✨', label: 'Deep review with Claude', run: () => { switchPanel('suggest'); $('#deep-review').click(); } });
  c.push({ group: 'Actions', ico: '⏱', label: 'Replay schema history (time machine)', run: () => openTimeMachine() });
  c.push({ group: 'Actions', ico: '⬇', label: 'Export diagram as PNG', run: () => exportDiagramPNG() });
  c.push({ group: 'Actions', ico: '⊙', label: 'Focus on selected table (cycle hops)', hint: '.', run: () => cycleFocus() });
  c.push({ group: 'Actions', ico: '↩', label: 'Undo layout change', hint: '⌘Z', run: () => undoLayout() });
  c.push({ group: 'Actions', ico: '?', label: 'Help & shortcuts', hint: '?', run: () => $('#help').classList.add('on') });
  c.push({ group: 'Appearance', ico: '◐', label: 'Theme: System', run: () => applyThemePref('system') });
  c.push({ group: 'Appearance', ico: '☀', label: 'Theme: Light', run: () => applyThemePref('light') });
  c.push({ group: 'Appearance', ico: '☾', label: 'Theme: Dark', run: () => applyThemePref('dark') });
  if (diff) for (const t of diff.tables) c.push({ group: 'Tables', ico: '▤', label: t.name, hint: t.columns.length + ' cols', run: () => { if (!state.pos[t.name]) return; state.selected = t.name; applyHighlight(); updateInspector(); centerOn(t.name); } });
  // columns — only surfaced once you start typing (would flood the initial list)
  if (diff) for (const t of diff.tables) for (const col of t.columns) c.push({
    group: 'Columns', ico: '·', deep: true,
    label: `${t.name}.${col.name}`, hint: typeLabel(col),
    run: () => {
      selectAndFocus(t.name);
      setTimeout(() => {
        const row = document.querySelector(`.node[data-table="${CSS.escape(t.name)}"] .row[data-col="${CSS.escape(col.key)}"]`);
        if (row) { row.classList.add('row-flash'); setTimeout(() => row.classList.remove('row-flash'), 1800); }
      }, 380);
    },
  });
  if (state.payload) for (const v of [...state.payload.versions].reverse()) c.push({ group: 'Versions', ico: 'v', label: `View v${v.id} — ${v.reason}`, hint: new Date(v.at).toLocaleTimeString(), run: () => { state.target = v.id; if (state.base === v.id) state.base = null; state.liveTarget = (v.id === state.payload.current); if (!state.versionsOpen) { state.versionsOpen = true; saveUIPrefs(); } loadComparison({ focusChanges: true }); } });
  return c;
}
...
function fuzzyMatch(q, text) {
  ...
  let qi = 0, score = 0, last = -2; const ranges = [];
  for (let i = 0; i < t.length && qi < q.length; i++) {
    if (t[i] === q[qi]) { score += (i === last + 1 ? 3 : 1) + (i === 0 ? 4 : 0); ranges.push(i); last = i; qi++; }
  }
...
function hlLabel(label, ranges) {
  ...
  for (let i = 0; i < label.length; i++) out += set.has(i) ? `<b>${escapeHtml(label[i])}</b>` : escapeHtml(label[i]);
  return out;
}
...
function renderPalette() {
  ...
  if (!scored.length) { list.innerHTML = '<div class="pl-empty">No matches</div>'; return; }
  let html = '', lastGroup = null;
  scored.forEach((s, i) => {
    const g = q ? 'Results' : s.it.group;
    if (g !== lastGroup) { html += `<div class="pl-group">${g}</div>`; lastGroup = g; }
  ...
  list.querySelectorAll('.pl-item').forEach((el) => {
    el.addEventListener('mousemove', () => { palette.sel = Number(el.dataset.i); markSel(); });
    el.addEventListener('click', () => runPalette(Number(el.dataset.i)));
  });
}
function markSel() {
  $('#palette-list').querySelectorAll('.pl-item').forEach((el, i) => el.classList.toggle('sel', i === palette.sel));
  const cur = $('#palette-list').querySelector('.pl-item.sel');
  if (cur) cur.scrollIntoView({ block: 'nearest' });
}
function runPalette(i) {
  const s = palette.filtered[i]; if (!s) return;
  closePalette();
  s.it.run();
}
function openPalette() {
  palette.items = paletteCommands(); palette.sel = 0;
  $('#palette').classList.add('on');
  const q = $('#palette-q'); q.value = ''; renderPalette();
  q.focus();
}
function closePalette() { $('#palette').classList.remove('on'); }
function setupPalette() {
  const q = $('#palette-q');
  q.addEventListener('input', () => { palette.sel = 0; renderPalette(); });
  q.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); palette.sel = Math.min(palette.filtered.length - 1, palette.sel + 1); markSel(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); palette.sel = Math.max(0, palette.sel - 1); markSel(); }
...
const TOUR_STEPS = [
  { sel: '#stage', text: 'This is your schema, live. Drag the background to pan, scroll to zoom, drag tables to arrange. Save the schema file and the diagram refreshes instantly.' },
  { sel: '#versions', text: 'Each migration is a version (or each save, in demo mode). Latest follows your Drizzle schema. Click a version to zoom into what changed. Collapse ▾ to show Latest only.' },
  { sel: '#search-launch', text: '⌘K is command central: fuzzy-search tables, columns, and every action. Press ? anytime for the full cheatsheet.' },
];
let tourIdx = 0;
function tourShow(i) {
  const step = TOUR_STEPS[i]; if (!step) return tourEnd();
  tourIdx = i;
  const target = document.querySelector(step.sel);
  const wrap = $('#tour'); const card = $('#tour-card'); const ring = $('#tour-ring');
  wrap.hidden = false;
  ...
  if (step.sel === '#stage') { cx = r.left + r.width / 2 - cw / 2; cy = r.top + 90; }
  cy = Math.min(Math.max(cy, 70), innerHeight - ch - 16);
  card.style.left = cx + 'px'; card.style.top = cy + 'px';
}
function tourEnd() {
  $('#tour').hidden = true;
  try { localStorage.setItem('dbd:toured', '1'); } catch {}
}
function maybeStartTour() {
  try { if (localStorage.getItem('dbd:toured')) return; } catch {}
  if (!nodeEls.size) return; // wait for a schema
  setTimeout(() => tourShow(0), 700);
}
function setupTour() {
  $('#tour-next').addEventListener('click', () => tourIdx >= TOUR_STEPS.length - 1 ? tourEnd() : tourShow(tourIdx + 1));
...
function setupHelp() {
  ...
  modal.addEventListener('click', (e) => { if (e.target.id === 'help') close(); });
}
...
function setupSidebar() {
  ...
  const prefs = loadUIPrefs();
  if (typeof prefs.versionsOpen === 'boolean') state.versionsOpen = prefs.versionsOpen;
  const applyVersionsOpen = () => {
    $('#versions-sec')?.classList.toggle('collapsed', !state.versionsOpen);
    const tog = $('#versions-toggle');
    if (tog) {
      tog.setAttribute('aria-expanded', state.versionsOpen ? 'true' : 'false');
      tog.title = state.versionsOpen ? 'Collapse versions — show Latest only' : 'Expand all versions';
    }
    if (state.payload) renderRail();
  };
  $('#versions-toggle')?.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
