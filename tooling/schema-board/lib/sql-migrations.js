...
function rebuildRelations(schema) {
  ...
  for (const t of schema.tables) {
    for (const c of t.columns) {
      if (c.references && c.references.table) {
        relations.push({
          from: t.name,
          fromColumn: c.name,
          fromKey: c.key,
...
function splitStatements(src) {
  const cleaned = stripSqlComments(src)
  ...
  while (i < n) {
    ...
    if (c === "'") {
      ...
      while (i < n) {
        cur += cleaned[i];
        if (cleaned[i] === "'" && cleaned[i + 1] === "'") { cur += cleaned[++i]; i++; continue; }
...
function parseTypeAndFlags(rest) {
  ...
  if (!matched) {
    ...
    if (m) { type = m[1].toLowerCase(); i = m[0].length; }
    else type = 'unknown';
  }

  let after = rest.slice(i).trim();
  ...
  // flags
  const flags = after;
  if (/\bPRIMARY\s+KEY\b/i.test(flags)) { col.pk = true; col.notNull = true; }
  ...
  const defM = flags.match(/\bDEFAULT\s+((?:'(?:[^']|'')*')|(?:"(?:[^"]|"")*")|(?:\([^\)]*\))|(?:[^\s,]+(?:\s+[^\s,]+)*?)(?=\s+(?:NOT\s+NULL|NULL|PRIMARY|UNIQUE|REFERENCES|CHECK|COLLATE)|$))/i);
  if (defM) {
    let d = defM[1].trim();
  ...
  const refM = flags.match(/\bREFERENCES\s+((?:"[^"]+"|[A-Za-z_][\w$]*)(?:\s*\.\s*(?:"[^"]+"|[A-Za-z_][\w$]*))?)\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)/i);
  if (refM) {
    const q = parseQualifiedName(refM[1]);
    const onDel = flags.match(/\bON\s+DELETE\s+(cascade|set\s+null|set\s+default|restrict|no\s+action)/i);
    col.references = {
      tableVar: q.name,
      table: q.name,
...
function parseColumnDef(line) {
  ...
  if (!line || /^(CONSTRAINT|CHECK|PRIMARY\s+KEY|UNIQUE|FOREIGN\s+KEY)/i.test(line)) return null;
...
function applyCreateTable(schema, stmt) {
  const m = stmt.match(/^CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([\s\S]+?)\s*\(([\s\S]*)\)\s*$/i);
  if (!m) return false;
  ...
  for (const part of splitTopLevelComma(m[2])) {
    ...
    if (col) columns.push(col);
      ...
      if (pk) {
        ...
        for (const c of columns) if (names.includes(c.key)) { c.pk = true; c.notNull = true; }
      }
      ...
      const fk = part.match(/FOREIGN\s+KEY\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)\s*REFERENCES\s+((?:"[^"]+"|[A-Za-z_][\w$]*)(?:\s*\.\s*(?:"[^"]+"|[A-Za-z_][\w$]*))?)\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)(?:\s*ON\s+DELETE\s+(cascade|set\s+null|set\s+default|restrict|no\s+action))?/i);
      if (fk) {
...
function applyDropTable(schema, stmt) {
  ...
  for (const part of m[1].split(',')) {
    ...
    schema.tables = schema.tables.filter((t) => !(t.name === q.name && (!q.schema || t.schema === q.schema)));
  }
  return true;
}

function applyAddColumn(schema, stmt) {
  const m = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+ADD\s+COLUMN\s+(?:IF\s+NOT\s+EXISTS\s+)?([\s\S]+)$/i);
  if (!m) return false;
...
function applyDropColumn(schema, stmt) {
  const m = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+DROP\s+COLUMN\s+(?:IF\s+EXISTS\s+)?(?:"([^"]+)"|([A-Za-z_][\w$]*))/i);
  if (!m) return false;
  const table = findTable(schema, m[1]);
  if (!table) return true;
  const name = m[2] || m[3];
...
function applyRenameColumn(schema, stmt) {
  const m = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+RENAME\s+COLUMN\s+(?:"([^"]+)"|([A-Za-z_][\w$]*))\s+TO\s+(?:"([^"]+)"|([A-Za-z_][\w$]*))/i);
  if (!m) return false;
  const table = findTable(schema, m[1]);
  if (!table) return true;
  const from = m[2] || m[3];
  ...
  if (col) { col.key = to; col.name = to; }
  return true;
}

function applyRenameTable(schema, stmt) {
  const m = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+RENAME\s+TO\s+(?:"([^"]+)"|([A-Za-z_][\w$]*))/i);
  if (!m) return false;
...
function applyAlterColumn(schema, stmt) {
  const m = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+ALTER\s+COLUMN\s+(?:"([^"]+)"|([A-Za-z_][\w$]*))\s+([\s\S]+)$/i);
  if (!m) return false;
  const table = findTable(schema, m[1]);
  if (!table) return true;
  ...
  if (/^SET\s+NOT\s+NULL\b/i.test(rest)) { col.notNull = true; return true; }
  if (/^DROP\s+NOT\s+NULL\b/i.test(rest)) { col.notNull = false; return true; }
  const setDef = rest.match(/^SET\s+DEFAULT\s+([\s\S]+)$/i);
  if (setDef) { col.default = setDef[1].trim().replace(/;$/, ''); return true; }
  if (/^DROP\s+DEFAULT\b/i.test(rest)) { col.default = null; return true; }
  return true;
}

function applyAddForeignKey(schema, stmt) {
  // ALTER TABLE … ADD CONSTRAINT … FOREIGN KEY (col) REFERENCES …
  const m = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+ADD\s+CONSTRAINT\s+(?:"[^"]+"|[A-Za-z_][\w$]*)\s+FOREIGN\s+KEY\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)\s*REFERENCES\s+((?:"[^"]+"|[A-Za-z_][\w$]*)(?:\s*\.\s*(?:"[^"]+"|[A-Za-z_][\w$]*))?)\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)(?:\s*ON\s+DELETE\s+(cascade|set\s+null|set\s+default|restrict|no\s+action))?/i);
  if (!m) {
    // without constraint name
    const m2 = stmt.match(/^ALTER\s+TABLE\s+(?:IF\s+EXISTS\s+)?(.+?)\s+ADD\s+FOREIGN\s+KEY\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)\s*REFERENCES\s+((?:"[^"]+"|[A-Za-z_][\w$]*)(?:\s*\.\s*(?:"[^"]+"|[A-Za-z_][\w$]*))?)\s*\(\s*(?:"([^"]+)"|([A-Za-z_][\w$]*))\s*\)(?:\s*ON\s+DELETE\s+(cascade|set\s+null|set\s+default|restrict|no\s+action))?/i);
    if (!m2) return false;
...
function applyStatement(schema, stmt) {
  ...
  if (/^(CREATE|DROP|ALTER)\s+(EXTENSION|SCHEMA|ROLE|POLICY|INDEX|UNIQUE\s+INDEX|FUNCTION|PROCEDURE|TRIGGER|TYPE|VIEW|MATERIALIZED|SEQUENCE)\b/i.test(s)) return false;
  if (/^GRANT\b|^REVOKE\b|^COMMENT\b|^ANALYZE\b|^VACUUM\b|^SELECT\b|^INSERT\b|^UPDATE\b|^DELETE\b|^DO\b/i.test(s)) return false;
  if (/\bENABLE\s+ROW\s+LEVEL\s+SECURITY\b|\bFORCE\s+ROW\s+LEVEL\s+SECURITY\b|\bDISABLE\s+ROW\s+LEVEL\s+SECURITY\b/i.test(s)) return false;
  ...
  if (/^ALTER\s+TABLE\b/i.test(s) && /\bDROP\s+COLUMN\b/i.test(s)) return applyDropColumn(schema, s);
  if (/^ALTER\s+TABLE\b/i.test(s) && /\bRENAME\s+COLUMN\b/i.test(s)) return applyRenameColumn(schema, s);
  if (/^ALTER\s+TABLE\b/i.test(s) && /\bRENAME\s+TO\b/i.test(s)) return applyRenameTable(schema, s);
  if (/^ALTER\s+TABLE\b/i.test(s) && /\bALTER\s+COLUMN\b/i.test(s)) return applyAlterColumn(schema, s);
  if (/^ALTER\s+TABLE\b/i.test(s) && /\bFOREIGN\s+KEY\b/i.test(s)) return applyAddForeignKey(schema, s);
  return false;
}
  ...
  return JSON.stringify({
    tables: (schema.tables || []).map((t) => ({
      ...
      cols: t.columns.map((c) => [
        c.key, c.type, c.pk, c.notNull, c.unique, c.default, c.length ?? '',
...
function buildMigrationVersions(migrationsDir) {
  ...
  for (const filePath of files) {
    ...
    let sql = '';
    try { sql = fs.readFileSync(filePath, 'utf8'); } catch { continue; }
