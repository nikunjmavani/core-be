...
function colSignature(c) {
  return JSON.stringify({
    type: c.type, pk: c.pk, notNull: c.notNull, unique: c.unique,
    default: c.default, isEnum: c.isEnum, array: c.array,
    references: c.references ? { table: c.references.table, column: c.references.column, onDelete: c.references.onDelete } : null,
...
function diffColumns(oldCols = [], newCols = [], changelog, tableName) {
  ...
  for (const c of newCols) {
    ...
    if (!prev) {
      ...
      changelog.push({ kind: 'column-added', table: tableName, column: c.name, detail: describeColumn(c) });
    } else if (colSignature(prev) !== colSignature(c)) {
      const changes = [];
      if (prev.type !== c.type) changes.push(`type ${prev.type} → ${c.type}`);
      if ((prev.length ?? null) !== (c.length ?? null)) changes.push(`length ${prev.length ?? '∅'} → ${c.length ?? '∅'}`);
      ...
      if (prev.default !== c.default) changes.push(`default ${prev.default ?? '∅'} → ${c.default ?? '∅'}`);
      ...
      const delta = {};
      if (prev.type !== c.type || (prev.length ?? null) !== (c.length ?? null) || !!prev.array !== !!c.array) delta.type = true;
      ...
      changelog.push({ kind: 'column-modified', table: tableName, column: c.name, detail: changes.join(', ') });
    } else {
  ...
  for (const c of oldCols) {
    if (!byKeyNew.has(c.key)) {
      ...
      changelog.push({ kind: 'column-removed', table: tableName, column: c.name, detail: describeColumn(c) });
    }
  }
  return result;
}
...
function diffSchemas(oldSchema, newSchema) {
  ...
  for (const t of oldSchema?.tables || []) {
    if (!newTables.has(t.name)) {
      tables.push({ ...t, status: 'removed', columns: t.columns.map((c) => ({ ...c, status: 'removed' })) });
  ...
  for (const e of newSchema.enums || []) {
    ...
    if (!prev) {
      changelog.push({ kind: 'enum-added', enum: e.name, values: e.values, detail: e.values.join(', ') });
    } else {
      ...
      if (added.length || removed.length) {
        ...
        changelog.push({ kind: 'enum-modified', enum: e.name, added, removed, detail: parts.join('  ') });
      }
    }
  }
  for (const e of oldSchema?.enums || []) {
    if (!newEnums.has(e.name)) changelog.push({ kind: 'enum-removed', enum: e.name, detail: `${e.values.length} values` });
  }
  ...
  const counts = {
    ...
    enumsChanged: changelog.filter((c) => c.kind && c.kind.startsWith('enum-')).length,
  };
  return { tables, relations: newSchema.relations, enums: newSchema.enums, dialect: newSchema.dialect, changelog, counts };
}

module.exports = { diffSchemas, describeColumn };
