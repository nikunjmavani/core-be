...
function splitTopLevel(src, sep = ',') {
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (c === sep && depth === 0 && angle === 0) { parts.push(cur); cur = ''; continue; }
    cur += c;
  }
...
function parseChain(expr) {
  expr = expr.trim();
  const typeMatch = expr.match(/^([A-Za-z_$][\w$]*)\s*\(/);
  if (!typeMatch) {
    // e.g. `roleEnum('role')` handled above; bare identifier fallback
    const id = expr.match(/^([A-Za-z_$][\w$]*)/);
    return { type: id ? id[1] : expr, args: '', methods: [] };
  }
  const type = typeMatch[1];
  let i = expr.indexOf('(', typeMatch.index);
  let close = matchBracket(expr, i);
  const args = expr.slice(i + 1, close);
  const methods = [];
  let rest = expr.slice(close + 1);
  while (true) {
...
function parseEnums(src) {
  const enums = [];
  const varByName = {};
  const re = new RegExp(`(?:export\\s+)?const\\s+([A-Za-z_$][\\w$]*)\\s*=\\s*(${ENUM_FNS.join('|')})\\s*\\(`, 'g');
  let m;
  while ((m = re.exec(src))) {
    const varName = m[1];
    ...
    if (args[1]) {
      ...
      if (arrMatch) {
        values = splitTopLevel(arrMatch[1]).map((v) => v.trim().replace(/^['"`]|['"`]$/g, '')).filter(Boolean);
      }
    }
...
function parseColumns(objSrc, ctx) {
  ...
  for (const entry of entries) {
    ...
    for (const meth of chain.methods) {
      switch (meth.name) {
        case 'primaryKey': col.pk = true; col.notNull = true; break;
        case 'notNull': col.notNull = true; break;
        case 'unique': col.unique = true; break;
        case 'array': col.array = true; break;
        case 'default':
          col.default = meth.args.trim();
          break;
        case 'defaultNow': col.default = 'now()'; break;
        case 'defaultRandom': col.default = 'random()'; break;
        case '$defaultFn':
        case '$default': col.default = 'fn()'; break;
        case 'references': {
          const refM = meth.args.match(/=>\s*([A-Za-z_$][\w$]*)\s*\.\s*([A-Za-z_$][\w$]*)/);
          if (refM) {
            const onDelete = meth.args.match(/onDelete\s*:\s*['"`]([^'"`]+)['"`]/);
            col.references = { tableVar: refM[1], column: refM[2], onDelete: onDelete ? onDelete[1] : null };
          }
          break;
        }
      }
...
function parseTableExtras(extrasSrc, table) {
  ...
  while ((im = indexRe.exec(body))) {
    ...
    const onCols = onM
      ? [...onM[1].matchAll(/\.([A-Za-z_$][\w$]*)/g)].map((y) => y[1])
      : [];
  ...
  for (const m of body.matchAll(/\bforeignKey\s*\(([\s\S]*?)\)/g)) {
    ...
    if (cols && foreign) {
      ...
      localCols.forEach((lc) => {
        ...
        if (c && refTable && !c.references) c.references = { tableVar: refTable[1], column: refTable[2], onDelete: null };
      });
    }
  }
}
...
function parsePgSchemas(src) {
  const map = {};
  const re = /(?:export\s+)?const\s+([A-Za-z_$][\w$]*)\s*=\s*pgSchema\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/g;
  let m;
  ...
  for (const t of tables) {
    for (const c of t.columns) {
      if (c.references && c.references.table) {
        relations.push({
          from: t.name, fromColumn: c.name, fromKey: c.key,
          to: c.references.table, toColumn: c.references.column,
...
module.exports = { parseSchema, stripComments, matchBracket, splitTopLevel };
