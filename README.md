...
- **One-command automated setup:** **[docs/deployment/setup/setup-automation.md](docs/deployment/setup/setup-automation.md)** — `pnpm setup:infra` provisions Neon, Redis, S3, Sentry, Railway, GitHub. Double confirm. The script never deletes resources and never rolls back on failure; run `pnpm setup:infra:delete` to print the dashboard URLs and identifiers for manual cleanup.
...
- **Redis** — managed service (Railway Redis by default — Railway's `redis` database template — or AWS ElastiCache, Redis Cloud)
...
For one-command automated setup (Neon, Redis, S3, Sentry, Railway, GitHub), see [docs/deployment/setup/setup-automation.md](docs/deployment/setup/setup-automation.md). Edit `tooling/setup/setup.config.json`, run `pnpm setup`. **Infrastructure must be ready before auto-deploy** (push to dev/main) can succeed.
