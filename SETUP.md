...
```bash
pnpm setup --init              # optional: interactive config → setup.config.json + .setup-credentials template
$EDITOR .setup-credentials             # fill provider API keys (each line has a comment with the URL)
...
- **Config** lives in `tooling/setup/setup.config.json` (committed); **secrets** in `.setup-credentials` at root (gitignored).
