...
-- migration-safety: allow rename_column_or_table reason="Converts freshly-created empty append-only parents into partitioned parents inside the init migration."
-- migration-safety: allow drop_table reason="Drops only freshly-created empty legacy parents during init-time partition conversion."
...
--> statement-breakpoint
CREATE OR REPLACE FUNCTION infrastructure.ensure_monthly_range_partition(
  parent_schema text,
  parent_table text,
...
BEGIN
  parent_qualified := format('%I.%I', parent_schema, parent_table);
  partition_name := format('%s_%s', parent_table, to_char(month_start AT TIME ZONE 'UTC', 'YYYY_MM'));
  partition_qualified := format('%I.%I', parent_schema, partition_name);
  range_start := date_trunc('month', month_start AT TIME ZONE 'UTC') AT TIME ZONE 'UTC';
  ...
  SELECT EXISTS (
    ...
    WHERE n.nspname = parent_schema
      AND c.relname = parent_table
      AND c.relkind = 'p'
  ) INTO parent_is_partitioned;

  IF NOT parent_is_partitioned THEN
    RAISE NOTICE 'Skipping partition % — parent % is not partitioned', partition_name, parent_qualified;
    RETURN;
  END IF;

  IF to_regclass(partition_qualified) IS NOT NULL THEN
    RETURN;
  END IF;
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION infrastructure.drop_empty_monthly_partitions_before(
  parent_schema text,
  parent_table text,
  cutoff timestamptz
) RETURNS integer
...
BEGIN
  FOR child IN
    ...
    WHERE parent_ns.nspname = parent_schema
      ...
      AND c.relkind = 'r'
  LOOP
    suffix := substring(child.table_name from '(\d{4}_\d{2})$');
    IF suffix IS NULL THEN
      CONTINUE;
    END IF;

    partition_month := to_timestamp(suffix, 'YYYY_MM');
    partition_end := partition_month + interval '1 month';
    IF partition_end > cutoff THEN
      CONTINUE;
    END IF;

    EXECUTE format('SELECT count(*)::bigint FROM %I.%I', child.schema_name, child.table_name)
      INTO row_count;

    IF row_count = 0 THEN
      EXECUTE format('DROP TABLE IF EXISTS %I.%I', child.schema_name, child.table_name);
...
$$;
--> statement-breakpoint
ALTER TABLE audit.logs RENAME TO logs_legacy;
--> statement-breakpoint
CREATE TABLE audit.logs (
  LIKE audit.logs_legacy INCLUDING DEFAULTS INCLUDING CONSTRAINTS
) PARTITION BY RANGE (created_at);
--> statement-breakpoint
ALTER TABLE audit.logs DROP CONSTRAINT IF EXISTS logs_pkey;
--> statement-breakpoint
ALTER TABLE audit.logs ADD PRIMARY KEY (id, created_at);
--> statement-breakpoint
INSERT INTO audit.logs SELECT * FROM audit.logs_legacy;
--> statement-breakpoint
ALTER SEQUENCE audit.logs_id_seq OWNED BY audit.logs.id;
--> statement-breakpoint
DROP TABLE IF EXISTS audit.logs_legacy CASCADE;
--> statement-breakpoint
ALTER TABLE audit.logs
  ADD CONSTRAINT logs_actor_user_id_users_id_fk
  FOREIGN KEY (actor_user_id) REFERENCES auth.users(id) ON DELETE SET NULL;
--> statement-breakpoint
ALTER TABLE audit.logs
  ADD CONSTRAINT logs_target_user_id_users_id_fk
  FOREIGN KEY (target_user_id) REFERENCES auth.users(id) ON DELETE SET NULL;
--> statement-breakpoint
ALTER TABLE audit.logs
  ADD CONSTRAINT logs_organization_id_organizations_id_fk
  FOREIGN KEY (organization_id) REFERENCES tenancy.organizations(id) ON DELETE SET NULL;
--> statement-breakpoint
CREATE INDEX idx_audit_logs_org_created ON audit.logs (organization_id, created_at);
--> statement-breakpoint
CREATE INDEX idx_audit_logs_actor_created ON audit.logs (actor_user_id, created_at);
--> statement-breakpoint
CREATE INDEX idx_audit_logs_resource ON audit.logs (resource_type, resource_id, created_at);
--> statement-breakpoint
CREATE INDEX idx_audit_logs_created_at ON audit.logs (created_at);
--> statement-breakpoint
CREATE INDEX idx_audit_logs_severity_created ON audit.logs (severity, created_at);
--> statement-breakpoint
CREATE INDEX idx_audit_logs_action_created ON audit.logs (action, created_at);
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE notify.notifications RENAME TO notifications_legacy;
--> statement-breakpoint
CREATE TABLE notify.notifications (
  LIKE notify.notifications_legacy INCLUDING DEFAULTS INCLUDING CONSTRAINTS
) PARTITION BY RANGE (created_at);
--> statement-breakpoint
ALTER TABLE notify.notifications DROP CONSTRAINT IF EXISTS notifications_pkey;
--> statement-breakpoint
ALTER TABLE notify.notifications ADD PRIMARY KEY (id, created_at);
--> statement-breakpoint
INSERT INTO notify.notifications SELECT * FROM notify.notifications_legacy;
--> statement-breakpoint
ALTER SEQUENCE notify.notifications_id_seq OWNED BY notify.notifications.id;
--> statement-breakpoint
DROP TABLE IF EXISTS notify.notifications_legacy CASCADE;
--> statement-breakpoint
...
ALTER TABLE notify.notifications
  ...
  FOREIGN KEY (organization_id) REFERENCES tenancy.organizations(id) ON DELETE SET NULL;
--> statement-breakpoint
CREATE INDEX idx_notifications_public_id ON notify.notifications (public_id);
--> statement-breakpoint
CREATE INDEX idx_notifications_user_read ON notify.notifications (user_id, is_read, created_at);
--> statement-breakpoint
CREATE INDEX idx_notifications_org ON notify.notifications (organization_id, created_at);
--> statement-breakpoint
CREATE INDEX idx_notifications_type ON notify.notifications (type, created_at);
--> statement-breakpoint
CREATE INDEX idx_notifications_created ON notify.notifications (created_at);
--> statement-breakpoint
ALTER TABLE notify.notifications ENABLE ROW LEVEL SECURITY;
--> statement-breakpoint
CREATE POLICY notifications_tenant_isolation ON notify.notifications
  USING (
    (
...
--> statement-breakpoint
SELECT infrastructure.ensure_monthly_range_partition('audit', 'logs', 'created_at', date_trunc('month', now()));
--> statement-breakpoint
SELECT infrastructure.ensure_monthly_range_partition('audit', 'logs', 'created_at', date_trunc('month', now()) + interval '1 month');
--> statement-breakpoint
SELECT infrastructure.ensure_monthly_range_partition('notify', 'notifications', 'created_at', date_trunc('month', now()));
--> statement-breakpoint
SELECT infrastructure.ensure_monthly_range_partition('notify', 'notifications', 'created_at', date_trunc('month', now()) + interval '1 month');
--> statement-breakpoint
DO $$
DECLARE
