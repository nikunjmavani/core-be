...
4. For any change under `src/`, the **layered documentation system** also applies — see **[docs/reference/architecture/documentation-system.md](docs/reference/architecture/documentation-system.md)** and the auto-trigger rule **[`.cursor/rules/feature-doc-maintainer-sync.mdc`](.cursor/rules/feature-doc-maintainer-sync.mdc)**. The hard gate (`pnpm features:check:strict`) is monotonic: missing-token counts may decrease but may not increase.
...
6. Before opening a pull request, ensure these pass (pre-commit runs the same sync checks locally):
   ...
   Same checks individually: `pnpm validate`, `pnpm validate:domain`, `pnpm routes:catalog:check`, `pnpm features:check:strict`, `pnpm db:migrate:lint`, `pnpm tool:sync-env-example`, `pnpm test`. Static CI quality slice (no tests): `pnpm ci:quality`.
